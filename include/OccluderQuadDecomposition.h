...
class OccluderQuad {
 public:
  ...
  struct MeshVertex {
    ...
    // edge linked list with start point this vertex.. might be dirty for a
    // while
  ...
  struct MeshFace {
    ...
    void AssignStartEndApex(MeshEdgeFace* e, MeshVertex* v0, MeshVertex* v1,
                            MeshVertex* v2) {
  ...
  struct RectEdge {
   public:
    uint16_t startGridX = 0;
    uint16_t startGridY = 0;
    uint16_t endGridX = 0;
    uint16_t endGridY = 0;
  ...
  struct MeshQuad {
    ...
    bool MergeWith(MeshQuad* q, __m128* vertices, MeshVertex* MeshVertices,
                   bool IsTerrain, int& TotalMerged);
  };

 public:
  static void CheckRectangleQuad(MeshQuad* q, __m128* vertices);
  static bool FitTerrainSquareProperty(uint32_t nIdx, uint32_t nVert,
                                       int terrainGridAxisPoint);
  // in case of Close Wise input, triangle would be auto re-order to CCW
  static void DecomposeToQuad(OccluderBakeBuffer* pBakeBuffer,
                              const uint16_t* indices, unsigned int nIdx,
                              const float* inputVertices, int vertNum,
                              QuadTriIndex& qti, float quadAngle,
                              TerrainInput* terrain = nullptr,
                              bool checkedTerrain = false,
                              int terrainGridAxisPoint = 0);

  static void AllowPlanarQuadMerge(bool allow);
  static void SetTerrainGridOptimization(int value);

  static void HandleSquareTerrainInput(
      CullingEngine::OccluderBakeBuffer* pBakeBuffer, const uint16_t* indices,
      unsigned int nIdx, const float* inputVertices, int vertNum,
      QuadTriIndex& qti, float quadAngle, int terrainGridWidth);
  ...
  static unsigned short* CullingEngineMeshBake(
 ...
 private:
  static bool MergeQuad(MeshQuad* q0, MeshQuad* q1, __m128* points,
                        MeshVertex* Vertices, bool isTerrain, int& totalMerged);
  static void ConfigControlParameters(bool isTerrain);
  static void SetRectangleDegreeToZero(
      CullingEngine::OccluderQuad::MeshVertex* Vertices, uint32_t nVert);
...
class OccluderBakeBuffer {
 private:
  ...
  std::vector<CullingEngine::OccluderQuad::MeshQuad> mQuads;
  std::vector<CullingEngine::OccluderQuad::MeshVertex> mVertices;

 public:
  std::vector<CullingEngine::OccluderQuad::MeshFace> mFaces;
  uint16_t* GetIndicesBySize(uint32_t size);

  __m128* GetPointsBySize(uint32_t size);

  CullingEngine::OccluderQuad::MeshFace* GetFaceBySize(uint32_t size);

  CullingEngine::OccluderQuad::MeshQuad* GetQuadBySize(uint32_t size);

  CullingEngine::OccluderQuad::MeshVertex* GetVertexBySize(uint32_t size);
  ~OccluderBakeBuffer();
  OccluderBakeBuffer() {
  ...
  const uint16_t* ReIndex(const uint16_t* indices, const float* inputVertices,

...
class TimerWheelNode {
	...
	bool is_pinned() const {
		return (flags_ & kFlagPinned) != 0;
	}
	bool is_irq_safe() const {
		return (flags_ & kFlagIrqSafe) != 0;
	}

	void set_user_data(void* data) {
		user_data_ = data;
	}
	void set_expires(TimePoint t) {
		expires_.store(t);
	}
	void set_flags(uint32_t f) {
		flags_ = f;
	}
	void add_flag(uint32_t f) {
		flags_ |= f;
	}
	...
	void set_bucket_iterator(BucketIterator it) {
		bucket_it_ = it;
	}
	BucketIterator bucket_iterator() const {
		return bucket_it_;
	}
	void clear_bucket_iterator() {
		bucket_it_ = BucketIterator();
	}
...
class TimerWheel {
	...
	static constexpr int64_t kLevelGran(int n) {
		return 1LL << (n * kLvlClkShift);
	}
	...
	int64_t current_jiffy() const {
		return jiffies_.load();
	}
	size_t active_count() const {
		size_t count = 0;
		for (int i = 0; i < kWheelSize; ++i)
			count += pending_[i].load();
		return count;
	}
	...
	static constexpr int buckets_per_level() {
		return kLvlSize;
	}
	static constexpr int64_t level_granularity_ms(int level) {
		return kLevelGran(level);
	}
	static int level_for_ms(int64_t ms) {
		return calc_level(ms);
	}

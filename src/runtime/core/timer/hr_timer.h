...
class HrTimerNode {
	...
	TimePoint expire_time() const {
		return expires_.load();
	}

	using MapType = std::multimap<TimePoint, HrTimerNode*>;
	using MapIterator = MapType::iterator;

	void set_linked_queue_iterator(MapIterator it) {
		queue_it_ = it;
	}
	MapIterator linked_queue_iterator() const {
		return queue_it_;
	}
	void clear_linked_queue_iterator() {
	...
	TimePoint softexpires() const {
		return softexpires_.load();
	}
	ClockId clock_id() const {
		return clock_id_;
	}
	...
	bool is_absolute() const {
		return mode_is_absolute(mode_);
	}
	bool is_relative() const {
		return mode_is_relative(mode_);
	}
	bool is_repeating() const {
		return mode_is_repeating(mode_);
	}
	bool is_one_shot() const {
		return mode_is_one_shot(mode_);
	}
	bool is_soft() const {
		return mode_is_soft(mode_);
	}
	bool is_hard() const {
		return mode_is_hard(mode_);
	}
...
class HrTimerSleeper {
	...
	const HrTimerNode& timer() const {
		return timer_;
	}
	bool is_sleeping() const {
		return sleeping_.load();
	}
...
class HrTimerManager {
	...
	void setup_timer(HrTimerNode* timer,
	...
	void setup_timer_on_stack(HrTimerNode* timer,
							  HrTimerNode::Callback callback,
							  ClockId clock_id,
							  TimerMode mode) {
	...
	void start_range_ns(HrTimerNode* timer,

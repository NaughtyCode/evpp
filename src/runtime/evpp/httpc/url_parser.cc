...
int URLParser::parse(const string& url_s) {
    ...
    if (it != url_s.end()) {
        ...
        transform(url_s.begin(), it, back_inserter(schema), [](int c) { return std::tolower(c); }); // protocol is icase
    ...
    transform(last_it, it, back_inserter(host), [](int c) { return std::tolower(c); }); // host is icase

...
bool Service::initSSL(bool force_enable) {
	...
	SSL_CTX_set_options(ctx,
						SSL_OP_SINGLE_DH_USE | SSL_OP_SINGLE_ECDH_USE |
							SSL_OP_NO_SSLv2 /*disable SSLv2*/ | SSL_OP_NO_SSLv3 /*disable SSLv3*/ |
	...
	if (1 != SSL_CTX_use_certificate_chain_file(ctx, certificate_chain_file_.c_str())) {
		ENGINE_LOG_ERROR(engine::GetLogger(),
						 "Load certificate chain file({})failed.",
						 certificate_chain_file_.c_str());
		ERR_print_errors_fp(stderr);
		EC_KEY_free(ecdh);
		SSL_CTX_free(ctx);
		return false;
	...
	if (1 != SSL_CTX_use_PrivateKey_file(ctx, private_key_file_.c_str(), SSL_FILETYPE_PEM)) {
		ENGINE_LOG_ERROR(
			engine::GetLogger(), "Load private key file({})failed.", private_key_file_.c_str());
		ERR_print_errors_fp(stderr);
		EC_KEY_free(ecdh);
		SSL_CTX_free(ctx);
		return false;
...
void Service::DefaultHandleRequest(const ContextPtr& ctx) {
	...
	if (default_callback_) {
		...
		evhttp_send_reply(
			ctx->req(), HTTP_BADREQUEST, g_http_code_string[HTTP_BADREQUEST], nullptr);
	}
}
...
void Service::SendReply(const ContextPtr& ctx, const std::string& response_data) {
	...
	auto f = [this, response]() {
		...
		assert(listen_loop_->IsInLoopThread());
		ENGINE_LOG_TRACE(engine::GetLogger(),
						 "this={} send reply in listening thread. evhttp_={}",
						 (void*) this,
						 (void*) evhttp_);
		...
		if (!response->buffer) {
			evhttp_send_reply(x->req(), http_code, g_http_code_string[http_code], nullptr);
			return;
		}

		evhttp_send_reply(x->req(), http_code, g_http_code_string[http_code], response->buffer);
	};

	// Forward this response sending task to HTTP listening thread
	if (listen_loop_->IsRunning()) {
		ENGINE_LOG_TRACE(engine::GetLogger(),
						 "this={} dispatch this SendReply to listening thread",
						 (void*) this);

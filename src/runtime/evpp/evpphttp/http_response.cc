...
void HttpResponse::add_date(Buffer& buf) {
	...
	cur_p = &cur;
#endif
	if (strftime(date, sizeof(date), "Date: %a, %d %b %Y %H:%M:%S GMT\r\n", cur_p) != 0) {
...
void HttpResponse::MakeHttpResponse(const int response_code,
	...
	if (response_code_iter == http_status_code.end()) {
		snprintf(
			status, sizeof status, "HTTP/%d.%d %d ", hp_.http_major, hp_.http_minor, response_code);
		buf.Append(status);
		buf.Append("Unknown");
	} else {
		snprintf(status,
	...
	if (need_body(response_code) &&
		header_field_value.find("Content-Type") == header_field_value.end()) {

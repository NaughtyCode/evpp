...
void TCPClient::DisconnectInLoop() {
	ENGINE_LOG_WARN(engine::GetLogger(),
					"TCPClient::DisconnectInLoop this={} remote_addr={}",
					(void*) this,
					remote_addr_);
	assert(loop_->IsInLoopThread());
	auto_reconnect_.store(false);
	...
	if (connector_) {
		if (connector_->IsConnected() || connector_->IsDisconnected()) {
			ENGINE_LOG_TRACE(engine::GetLogger(),
							 "this={} Nothing to do with connector_, Connector::status={}",
							 (void*) this,
							 connector_->status());
		} else {
...
void TCPClient::Reconnect() {
	ENGINE_LOG_TRACE(engine::GetLogger(),
					 "this={} Try to reconnect to {} in {}s again",
					 (void*) this,
					 remote_addr_,
...
void TCPClient::OnConnection(evpp_socket_t sockfd, const std::string& laddr) {
	if (sockfd < 0) {
		ENGINE_LOG_TRACE(engine::GetLogger(),
						 "this={} Failed to connect to {}. errno={} {}",
						 (void*) this,
						 remote_addr_,
						 EVPP_ERRNO,
						 strerror(EVPP_ERRNO));

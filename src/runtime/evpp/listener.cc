...
void Listener::HandleAccept() {
    ...
    if ((nfd = ::accept(fd_, sock::sockaddr_cast(&ss), &addrlen)) == -1) {
        ...
        if (serrno != EAGAIN && serrno != EINTR) {
            ENGINE_LOG_WARN(engine::GetLogger(), "{} bad accept {}", __FUNCTION__, strerror(serrno));
        }
        return;
    }
    ...
    ENGINE_LOG_TRACE(engine::GetLogger(), "this={} accepted a connection from {}, listen fd={}, client fd={}", (void*)this, raddr, fd_, nfd);

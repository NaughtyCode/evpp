...
#endif
	CLOUDENGINE_MEM_DELETE(pending_functors_);
...
void EventLoop::Init() {
	...
	this->pending_functors_ = CLOUDENGINE_MEM_NEW(boost::lockfree::queue<Functor*>, kPendingFunctorCount);
#elif defined(H_HAVE_CAMERON314_CONCURRENTQUEUE)
	this->pending_functors_ = CLOUDENGINE_MEM_NEW(moodycamel::ConcurrentQueue<Functor>);
#else
	this->pending_functors_ = CLOUDENGINE_MEM_NEW(std::vector<Functor>);
#endif
...
void EventLoop::InitNotifyPipeWatcher() {
	...
	watcher_.reset(CLOUDENGINE_MEM_NEW(PipeEventWatcher, this, std::bind(&EventLoop::DoPendingFunctors, this)));
...
void EventLoop::QueueInLoop(const Functor& cb) {
	...
	{
#ifdef H_HAVE_BOOST
		auto f = CLOUDENGINE_MEM_NEW(Functor, cb);
...
void EventLoop::QueueInLoop(Functor&& cb) {
	...
	{
#ifdef H_HAVE_BOOST
		auto f = CLOUDENGINE_MEM_NEW(Functor, std::move(cb));  // Deleted by consumer in DoPendingFunctors
		while (!pending_functors_->push(f)) {
		}
#elif defined(H_HAVE_CAMERON314_CONCURRENTQUEUE)

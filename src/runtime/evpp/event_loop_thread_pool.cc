...
bool EventLoopThreadPool::Start(bool wait_thread_started) {
	...
	ENGINE_LOG_TRACE(engine::GetLogger(),
					 "this={} thread_num={} base loop={} wait_thread_started={}",
					 (void*) this,
					 thread_num(),
					 (void*) base_loop_,
					 wait_thread_started);
	...
	std::shared_ptr<std::atomic<uint32_t>> started_count(CLOUDENGINE_MEM_NEW(std::atomic<uint32_t>, 0));
	std::shared_ptr<std::atomic<uint32_t>> exited_count(CLOUDENGINE_MEM_NEW(std::atomic<uint32_t>, 0));
	for (uint32_t i = 0; i < thread_num_; ++i) {
		auto prefn = [this, started_count]() {
			ENGINE_LOG_TRACE(engine::GetLogger(),
							 "a working thread started tid={}",
		...
		auto postfn = [this, exited_count]() {
			ENGINE_LOG_TRACE(engine::GetLogger(),
							 "a working thread exiting, tid={}",
							 std::hash<std::thread::id>{}(std::this_thread::get_id()));
			this->OnThreadExited(exited_count->fetch_add(1) + 1);
			return EventLoopThread::kOK;
		};

		EventLoopThreadPtr t(CLOUDENGINE_MEM_NEW(EventLoopThread));
...
void EventLoopThreadPool::Stop(bool wait_thread_exit) {
	ENGINE_LOG_TRACE(
		engine::GetLogger(), "this={} wait_thread_exit={}", (void*) this, wait_thread_exit);
...
void EventLoopThreadPool::OnThreadExited(uint32_t count) {
	...
	if (count == thread_num_) {
		status_.store(kStopped);
		ENGINE_LOG_TRACE(engine::GetLogger(),
						 "this={} this is the last thread stopped. Thread pool totally exited.",
						 (void*) this);
		if (stopped_cb_) {
			stopped_cb_();

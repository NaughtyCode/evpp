...
DNSResolver::~DNSResolver() {
	...
	if (evdns_cb_arg_) {
		CLOUDENGINE_MEM_DELETE(evdns_cb_arg_);
...
void DNSResolver::AsyncWait() {
	ENGINE_LOG_TRACE(engine::GetLogger(),
					 ...
					 std::hash<std::thread::id>{}(std::this_thread::get_id()),
					 (void*) this);
	timer_.reset(CLOUDENGINE_MEM_NEW(TimerEventWatcher, loop_, std::bind(&DNSResolver::OnTimeout, this), timeout_));

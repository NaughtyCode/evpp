...
bool ConfigManager::LoadMongoDbConfigFromFile(const std::string& path,
                                               MongoDbConfig& out) {
    std::string buf;
...
void ConfigManager::LoadMongoDbConfigsFromServer() {
    ...
    if (!server_config_.mongodb_dev.empty()) {
        mongo_dev_loaded_ = LoadMongoDbConfigFromFile(
            server_config_.mongodb_dev, mongo_dev_config_);
    }
    if (!server_config_.mongodb_public.empty()) {
        mongo_public_loaded_ = LoadMongoDbConfigFromFile(
            server_config_.mongodb_public, mongo_public_config_);
    }
}

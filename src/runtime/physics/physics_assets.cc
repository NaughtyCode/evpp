	...
	if (type == "box") {
		...
		if (df_arr.is_array() && df_arr.size() > 0) {
			...
			if (sample_count == 0) {
				...
				if (sample_count * sample_count != sample_count_file) {
					result.error =
						"height_field: sampleCount not specified and file size "
		...
		if (samples_arr.is_array()) {
			...
			for (size_t i = 0; i < samples_arr.size(); ++i) {
				samples.push_back(
					static_cast<float>(samples_arr[unsigned(i)].template get<double>()));
			}
			uint32_t sample_count = static_cast<uint32_t>(get_num("sampleCount", 0));
			if (sample_count == 0) {
				...
				if (static_cast<size_t>(sample_count) * sample_count != samples.size()) {
					result.error =
						"height_field: inline sampleCount not specified "
...
AssetLoadResult AssetLoader::LoadScene(const std::string& json_path,
	...
	if (ec) {
		result.error = "parse error in " + json_path + ": " + glz::format_error(ec, json);
		return result;
	}
	...
	if (asset.static_bodies.has_value()) {
		...
		if (!body_ids.empty()) {
			JPH::BodyInterface::AddState add_state =
				body_interface.AddBodiesPrepare(body_ids.data(), static_cast<int>(body_ids.size()));

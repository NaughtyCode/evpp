...
class PhysicsSystem {
	...
	const ScriptVM& GetScriptVM() const {
		return *script_vm_;
	}

	PhysicsScriptVM& GetPhysicsScriptVM() {
		return *script_vm_;
	}
	const PhysicsScriptVM& GetPhysicsScriptVM() const {
		return *script_vm_;
	}
	...
	quill::Logger* GetPhysicsLogger() const {
		return physics_thread_.GetLogger();
	}

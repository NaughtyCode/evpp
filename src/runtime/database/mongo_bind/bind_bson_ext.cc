...
int l_context_destroy(lua_State* L) {
	l_context_gc(L);
	return 0;
}
...
int l_string_destroy(lua_State* L) {
	l_string_gc(L);
	return 0;
}
...
int l_json_reader_destroy(lua_State* L) {
	l_json_reader_gc(L);
	return 0;
}
...
int l_json_data_reader_destroy(lua_State* L) {
	l_json_data_reader_gc(L);
	return 0;
}
...
int l_json_data_reader_ingest(lua_State* L) {
	...
	lua_pushboolean(L, reader && reader->Ingest(reinterpret_cast<const uint8_t*>(data), len));
	return 1;
}
...
int l_reader_destroy(lua_State* L) {
	l_reader_gc(L);
	return 0;
}
...
int l_writer_destroy(lua_State* L) {
	l_writer_gc(L);
	return 0;
}
...
int l_json_opts_new(lua_State* L) {
	...
	auto* p =
		new (std::nothrow) mongo::BsonJsonOpts(static_cast<mongo::BsonJsonMode>(mode), max_len);
	if (!p) {
		lua_pushnil(L);
		lua_pushstring(L, "allocation failure");
		lua_pushnil(L);
...
int l_json_opts_destroy(lua_State* L) {
	l_json_opts_gc(L);
	return 0;
}
...
int l_value_destroy(lua_State* L) {
	l_value_gc(L);
	return 0;
}

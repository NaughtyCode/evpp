...
int l_log_opts_destroy(lua_State* L) { l_log_opts_gc(L); return 0; }
...
int l_log_opts_get_max_level_for_component(lua_State* L) {
    auto* opts = GetUserdata<mongo::MongoStructuredLogOpts>(L, 1, kOptsMeta);
    auto component = static_cast<mongo::MongoStructuredLogComponent>(luaL_checkinteger(L, 2));
    lua_pushinteger(L, opts ? static_cast<lua_Integer>(opts->GetMaxLevelForComponent(component)) : 0);
    return 1;
}
...
int l_log_entry_new(lua_State* L) {
    ...
    if (!raw) { lua_pushnil(L); lua_pushstring(L, "invalid raw entry pointer"); return 2; }
    auto* e = new (std::nothrow) mongo::MongoStructuredLogEntry(raw);
    if (!e) { lua_pushnil(L); lua_pushstring(L, "allocation failure"); return 2; }
    auto** ud = NewUserdata<mongo::MongoStructuredLogEntry>(L, kEntryMeta);
    *ud = e;
    return 1;
}

int l_log_entry_destroy(lua_State* L) { l_log_entry_gc(L); return 0; }

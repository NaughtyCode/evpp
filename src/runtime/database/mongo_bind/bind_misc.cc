...
int l_optional_new(lua_State* L) {
    ...
    if (!opt) { lua_pushnil(L); lua_pushstring(L, "allocation failure"); return 2; }
    auto** ud = NewUserdata<mongo::MongoOptional>(L, kOptionalMeta);
    *ud = opt;
    return 1;
}

int l_optional_destroy(lua_State* L) { l_optional_gc(L); return 0; }
...
const luaL_Reg* GetMongoOptionalLib() { return kOptionalLib; }

const luaL_Reg* GetMongoMiscLib() { return kMiscLib; }

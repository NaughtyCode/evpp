...
int l_sd_new(lua_State* L) {
    ...
    if (!raw) { lua_pushnil(L); lua_pushstring(L, "raw server_description pointer required"); return 2; }
    auto* sd = new (std::nothrow) mongo::MongoServerDescription(raw);
    if (!sd) { lua_pushnil(L); lua_pushstring(L, "allocation failure"); return 2; }
    auto** ud = NewUserdata<mongo::MongoServerDescription>(L, kSdMeta);
    *ud = sd;
    return 1;
}

int l_sd_destroy(lua_State* L) { l_sd_gc(L); return 0; }
...
int l_sd_new_copy(lua_State* L) {
    ...
    if (!copy) { lua_pushnil(L); lua_pushstring(L, "copy failed"); return 2; }
    auto** ud = NewUserdata<mongo::MongoServerDescription>(L, kSdMeta);
    *ud = copy;
    return 1;
}
...
int l_sd_host(lua_State* L) {
    auto* sd = GetUserdata<mongo::MongoServerDescription>(L, 1, kSdMeta);
    if (!sd) { lua_pushnil(L); return 1; }
    const auto* host = sd->Host();
    if (!host) { lua_pushnil(L); return 1; }
...
int l_sd_hello_response(lua_State* L) {
    ...
    if (!raw_bson) { lua_pushnil(L); return 1; }
...
int l_td_new(lua_State* L) {
    void* raw = lua_touserdata(L, 1);
    if (!raw) { lua_pushnil(L); lua_pushstring(L, "raw topology_description pointer required"); return 2; }
    auto* td = new (std::nothrow) mongo::MongoTopologyDescription(raw);
    if (!td) { lua_pushnil(L); lua_pushstring(L, "allocation failure"); return 2; }
    auto** ud = NewUserdata<mongo::MongoTopologyDescription>(L, kTdMeta);
    *ud = td;
    return 1;
}

int l_td_destroy(lua_State* L) { l_td_gc(L); return 0; }
...
int l_td_has_readable_server(lua_State* L) {
    auto* td = GetUserdata<mongo::MongoTopologyDescription>(L, 1, kTdMeta);
    auto* prefs = lua_isnoneornil(L, 2) ? nullptr
...
int l_td_servers(lua_State* L) {
    auto* td = GetUserdata<mongo::MongoTopologyDescription>(L, 1, kTdMeta);
    if (!td) { lua_pushnil(L); return 1; }
    size_t n;
    auto** servers = td->GetServers(&n);
    if (!servers) { lua_pushnil(L); return 1; }
...
const luaL_Reg* GetMongoServerDescriptionLib() { return kSdLib; }
const luaL_Reg* GetMongoTopologyDescriptionLib() { return kTdLib; }

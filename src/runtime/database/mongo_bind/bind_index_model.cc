...
int l_index_model_new(lua_State* L) {
    ...
    if (!keys) { lua_pushnil(L); lua_pushstring(L, "keys document required"); return 2; }
    auto* m = new (std::nothrow) mongo::MongoIndexModel(*keys, opts);
    if (!m) { lua_pushnil(L); lua_pushstring(L, "allocation failure"); return 2; }
    auto** ud = NewUserdata<mongo::MongoIndexModel>(L, kMetaName);
    *ud = m;
    return 1;
}

int l_index_model_destroy(lua_State* L) { l_index_model_gc(L); return 0; }
...
const luaL_Reg* GetMongoIndexModelLib() { return kLib; }

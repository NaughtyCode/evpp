...
int l_vi8c_new(lua_State* L) {
    ...
    if (!v) { lua_pushnil(L); lua_pushstring(L, "allocation failure"); return 2; }
...
int l_vi8c_destroy(lua_State* L) { l_vi8c_gc(L); return 0; }
...
int l_vi8_as_const(lua_State* L) {
    ...
    if (!cv) { lua_pushnil(L); lua_pushstring(L, "allocation failure"); return 2; }
    auto** ud = NewUserdata<mongo::BsonVectorInt8ConstView>(L, kVi8cMeta);
    *ud = cv;
    return 1;
}
...
int l_vi8_destroy(lua_State* L) { l_vi8_gc(L); return 0; }
...
int l_vf32c_destroy(lua_State* L) { l_vf32c_gc(L); return 0; }
...
int l_vf32_write(lua_State* L) {
    auto* v = GetUserdata<mongo::BsonVectorFloat32View>(L, 1, kVf32Meta);
    if (!lua_istable(L, 2) || !v) { lua_pushboolean(L, false); return 1; }
...
int l_vf32_destroy(lua_State* L) { l_vf32_gc(L); return 0; }
...
int l_vpb_unpack_bool(lua_State* L) {
    ...
    if (!buf) { lua_pushnil(L); lua_pushstring(L, "allocation failure"); return 2; }
...
int l_vpb_destroy(lua_State* L) { l_vpb_gc(L); return 0; }
...
int l_vpbw_pack_bool(lua_State* L) {
    ...
    auto offset = static_cast<size_t>(luaL_optinteger(L, 3, 0));
    auto* buf = new (std::nothrow) bool[count];
    if (!buf) { lua_pushboolean(L, false); return 1; }
    for (size_t i = 0; i < count; ++i) {
        lua_rawgeti(L, 2, (int)i + 1);
...
int l_vpbw_destroy(lua_State* L) { l_vpbw_gc(L); return 0; }
...
const luaL_Reg* GetBsonVectorInt8ConstViewLib() { return kVi8cLib; }
const luaL_Reg* GetBsonVectorInt8ViewLib() { return kVi8Lib; }
const luaL_Reg* GetBsonVectorFloat32ConstViewLib() { return kVf32cLib; }
const luaL_Reg* GetBsonVectorFloat32ViewLib() { return kVf32Lib; }
const luaL_Reg* GetBsonVectorPackedBitConstViewLib() { return kVpbLib; }
const luaL_Reg* GetBsonVectorPackedBitViewLib() { return kVpbwLib; }

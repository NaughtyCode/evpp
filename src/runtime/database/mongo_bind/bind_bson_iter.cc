...
int l_bson_iter_find_descendant(lua_State* L) {
    ...
    if (!desc) { lua_pushboolean(L, false); lua_pushstring(L, "allocation failure"); return 2; }
    bool ok = iter->FindDescendant(dotkey, desc);
    if (!ok) { delete desc; lua_pushboolean(L, false); return 1; }
    auto** ud = NewUserdata<mongo::BsonIter>(L, kMetaName);
    *ud = desc;
...
int l_bson_iter_as_dbref(lua_State* L) {
    ...
    if (collection) lua_pushlstring(L, collection, coll_len);
    else lua_pushnil(L);
...
int l_bson_iter_value(lua_State* L) {
    ...
    if (val) lua_pushlightuserdata(L, const_cast<void*>(val));
    else lua_pushnil(L);
    return 1;
}

...
int l_oidc_cred_new(lua_State* L) {
    ...
    if (!cred) { lua_pushnil(L); lua_pushstring(L, "credential creation failed"); return 2; }
...
int l_oidc_cred_destroy(lua_State* L) { l_oidc_cred_gc(L); return 0; }

int l_oidc_cred_get_access_token(lua_State* L) {
    auto* cred = GetUserdata<mongo::MongoOidcCredential>(L, 1, kCredMeta);
    if (!cred) { lua_pushnil(L); return 1; }
    const char* token = cred->GetAccessToken();
    if (token) lua_pushstring(L, token);
    else lua_pushnil(L);
    return 1;
}
...
int l_oidc_cred_get_expires_in(lua_State* L) {
    ...
    if (expires) lua_pushinteger(L, static_cast<lua_Integer>(*expires));
    else lua_pushnil(L);
    return 1;
}
...
int l_oidc_params_new(lua_State* L) {
    ...
    if (!raw) { lua_pushnil(L); lua_pushstring(L, "invalid params pointer"); return 2; }
...
int l_oidc_params_destroy(lua_State* L) { l_oidc_params_gc(L); return 0; }
...
int l_oidc_params_get_user_data(lua_State* L) {
    ...
    if (ud) lua_pushlightuserdata(L, ud);
    else lua_pushnil(L);
    return 1;
}
...
int l_oidc_params_get_timeout(lua_State* L) {
    ...
    if (timeout) lua_pushinteger(L, static_cast<lua_Integer>(*timeout));
    else lua_pushnil(L);
    return 1;
}
...
int l_oidc_cb_destroy(lua_State* L) { l_oidc_cb_gc(L); return 0; }
...
const luaL_Reg* GetMongoOidcCredentialLib()    { return kCredLib; }
const luaL_Reg* GetMongoOidcCallbackParamsLib() { return kParamsLib; }
const luaL_Reg* GetMongoOidcCallbackLib()       { return kCbLib; }

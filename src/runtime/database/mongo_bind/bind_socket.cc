...
int l_socket_destroy(lua_State* L) { l_socket_gc(L); return 0; }
...
int l_socket_new(lua_State* L) {
    ...
    if (!s) { lua_pushnil(L); lua_pushstring(L, "socket creation failed"); return 2; }
...
int l_socket_accept(lua_State* L) {
    ...
    if (!accepted) { lua_pushnil(L); return 1; }
    auto** ud = NewUserdata<mongo::MongoSocket>(L, kMetaName);
    *ud = accepted;
    return 1;
}
...
const luaL_Reg* GetMongoSocketLib() { return kLib; }

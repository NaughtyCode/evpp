...
int l_stream_destroy(lua_State* L) { l_stream_gc(L); return 0; }
...
int l_stream_new_socket(lua_State* L) {
    ...
    if (!socket) { lua_pushnil(L); lua_pushstring(L, "invalid socket"); return 2; }
...
int l_stream_write(lua_State* L) {
    ...
    if (!s) { lua_pushnil(L); lua_pushstring(L, "invalid stream"); return 2; }
...
int l_stream_read(lua_State* L) {
    ...
    if (!s) { lua_pushnil(L); lua_pushstring(L, "invalid stream"); return 2; }
    if (count == 0) { lua_pushstring(L, ""); return 1; }
    std::vector<char> buf(count);
    ssize_t nread = s->Read(buf.data(), count, min_bytes, timeout);
    if (nread < 0) { lua_pushnil(L); return 1; }
...
int l_stream_tls_handshake(lua_State* L) {
    ...
    const char* host = luaL_checkstring(L, 2);
    int32_t timeout = static_cast<int32_t>(luaL_checkinteger(L, 3));
    if (!s) { lua_pushboolean(L, false); lua_pushinteger(L, 0); return 2; }
...
int l_stream_get_socket(lua_State* L) {
    ...
    if (socket) lua_pushlightuserdata(L, socket);
    else lua_pushnil(L);
    return 1;
}
...
const luaL_Reg* GetMongoStreamLib() { return kLib; }

...
void MongoReadPrefs::SetTags(const BsonDocument& tags) {
	if (impl_ && impl_->prefs)
		mongoc_read_prefs_set_tags(impl_->prefs, static_cast<const bson_t*>(tags.RawBson()));
}

bool MongoReadPrefs::AddTag(const BsonDocument& tag) {
	if (!impl_ || !impl_->prefs) return false;
	mongoc_read_prefs_add_tag(impl_->prefs, static_cast<const bson_t*>(tag.RawBson()));
	return true;
}

int64_t MongoReadPrefs::GetMaxStalenessSeconds() const {
	return impl_ && impl_->prefs ? mongoc_read_prefs_get_max_staleness_seconds(impl_->prefs) : 0;
}

void MongoReadPrefs::SetMaxStalenessSeconds(int max_staleness_seconds) {
	if (impl_ && impl_->prefs)
		mongoc_read_prefs_set_max_staleness_seconds(impl_->prefs,
													static_cast<int64_t>(max_staleness_seconds));
}
...
void MongoReadPrefs::SetHedge(const BsonDocument& hedge) {
	if (impl_ && impl_->prefs)
		mongoc_read_prefs_set_hedge(impl_->prefs, static_cast<const bson_t*>(hedge.RawBson()));
}
...
MongoWriteConcern::MongoWriteConcern(MongoWriteConcern&& other) noexcept
	: impl_(std::move(other.impl_)) {
}
...
MongoReadConcern::MongoReadConcern(MongoReadConcern&& other) noexcept
	: impl_(std::move(other.impl_)) {
}

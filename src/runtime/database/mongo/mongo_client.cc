...
bool MongoClient::SetServerApi(const MongoServerApi& api, MongoError* error) {
	return impl_ && impl_->client &&
		   mongoc_client_set_server_api(impl_->client,
...
void MongoClient::SetSslOpts(const void* ssl_opts) {
	if (impl_ && impl_->client && ssl_opts) {
		mongoc_client_set_ssl_opts(impl_->client, static_cast<const mongoc_ssl_opt_t*>(ssl_opts));
	}
}
...
bool MongoClient::CommandSimple(const char* db_name,
...
void MongoClient::SetReadPrefs(const MongoReadPrefs& read_prefs) {
	if (impl_ && impl_->client)
		mongoc_client_set_read_prefs(
			impl_->client, static_cast<const mongoc_read_prefs_t*>(read_prefs.RawReadPrefs()));
}
...
void MongoClient::Reset() {
	if (impl_ && impl_->client) mongoc_client_reset(impl_->client);
}

bool MongoClient::ReadCommandWithOpts(const char* db_name,
									  const BsonDocument& command,
									  const MongoReadPrefs* read_prefs,
...
bool MongoClient::WriteCommandWithOpts(const char* db_name,
									   const BsonDocument& command,
									   const BsonDocument* opts,
									   BsonDocument* reply,
									   MongoError* error) {
...
bool MongoClient::ReadWriteCommandWithOpts(const char* db_name,
										   const BsonDocument& command,
										   const MongoReadPrefs* read_prefs,
										   const BsonDocument* opts,
										   BsonDocument* reply,
...
char** MongoClient::GetDatabaseNames(MongoError* error) {
	if (!impl_ || !impl_->client) return nullptr;
	return mongoc_client_get_database_names_with_opts(
		impl_->client, nullptr, error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}
...
MongoChangeStream* MongoClient::Watch(const BsonDocument& pipeline, const BsonDocument* opts) {
	...
	mongoc_change_stream_t* stream =
		mongoc_client_watch(impl_->client,
...
bool MongoClient::CommandSimpleWithServerId(const char* db_name,
											const BsonDocument& command,
											const MongoReadPrefs* read_prefs,
											uint32_t server_id,
											BsonDocument* reply,
...
bool MongoClient::CommandWithOpts(const char* db_name,
								  const BsonDocument& command,
								  const MongoReadPrefs* read_prefs,
								  const BsonDocument* opts,
								  BsonDocument* reply,
								  MongoError* error) {
	if (!impl_ || !impl_->client) return false;
	return mongoc_client_command_with_opts(
		impl_->client,
		db_name,
		static_cast<const bson_t*>(command.RawBson()),
		read_prefs ? static_cast<const mongoc_read_prefs_t*>(read_prefs->RawReadPrefs()) : nullptr,
...
void* MongoClient::SelectServer(bool for_writes, const MongoReadPrefs* prefs, MongoError* error) {
	if (!impl_ || !impl_->client) return nullptr;
	return mongoc_client_select_server(
		impl_->client,
		for_writes,
...
void** MongoClient::GetServerDescriptions(size_t* n) const {
	if (!impl_ || !impl_->client) return nullptr;
	return reinterpret_cast<void**>(mongoc_client_get_server_descriptions(impl_->client, n));
}

void MongoClient::ServerDescriptionsDestroyAll(void** sds, size_t n) {
	mongoc_server_descriptions_destroy_all(reinterpret_cast<mongoc_server_description_t**>(sds), n);
}

void* MongoClient::GetHandshakeDescription(uint32_t server_id,
										   const BsonDocument* opts,
										   MongoError* error) {
	if (!impl_ || !impl_->client) return nullptr;
...
void* MongoClient::GetGridfs(const char* db, const char* prefix, MongoError* error) {
	if (!impl_ || !impl_->client) return nullptr;
	return mongoc_client_get_gridfs(
		impl_->client, db, prefix, error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}

void MongoClient::SetStreamInitiator(void* initiator, void* user_data) {
	if (impl_ && impl_->client)
		mongoc_client_set_stream_initiator(
			impl_->client, reinterpret_cast<mongoc_stream_initiator_t>(initiator), user_data);
}

bool MongoClient::SetOidcCallback(const void* callback) {
	return impl_ && impl_->client &&
		   mongoc_client_set_oidc_callback(impl_->client,
										   static_cast<const mongoc_oidc_callback_t*>(callback));
}

bool MongoClient::AppendMetadata(const char* name, const char* version, const char* platform) {
	return impl_ && impl_->client &&
		   mongoc_client_append_metadata(impl_->client, name, version, platform);
}

void MongoClient::SetUsleepImpl(UsleepFunc func, void* user_data) {
	if (impl_ && impl_->client)
		mongoc_client_set_usleep_impl(
			impl_->client, reinterpret_cast<mongoc_usleep_func_t>(func), user_data);
}
...
bool MongoDatabase::HasCollection(const char* name, MongoError* error) {
	...
	return mongoc_database_has_collection(
		impl_->db, name, error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}
...
bool MongoCollection::CommandWithOpts(const BsonDocument& command,
									  ...
									  MongoError* error) {
	if (!impl_ || !impl_->coll) return false;
	return mongoc_collection_command_with_opts(
		impl_->coll,
		static_cast<const bson_t*>(command.RawBson()),
		read_prefs ? static_cast<const mongoc_read_prefs_t*>(read_prefs->RawReadPrefs()) : nullptr,
		opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
...
bool MongoCollection::ReadCommandWithOpts(const BsonDocument& command,
										  ...
										  MongoError* error) {
	if (!impl_ || !impl_->coll) return false;
	return mongoc_collection_read_command_with_opts(
		impl_->coll,
		static_cast<const bson_t*>(command.RawBson()),
		read_prefs ? static_cast<const mongoc_read_prefs_t*>(read_prefs->RawReadPrefs()) : nullptr,
		opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
...
bool MongoCollection::WriteCommandWithOpts(const BsonDocument& command,
										   ...
										   MongoError* error) {
	if (!impl_ || !impl_->coll) return false;
	return mongoc_collection_write_command_with_opts(
		impl_->coll,
		static_cast<const bson_t*>(command.RawBson()),
		opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
		reply ? static_cast<bson_t*>(reply->RawBson()) : nullptr,
...
bool MongoCollection::ReadWriteCommandWithOpts(const BsonDocument& command,
											   ...
											   MongoError* error) {
	if (!impl_ || !impl_->coll) return false;
	return mongoc_collection_read_write_command_with_opts(
		impl_->coll,
		static_cast<const bson_t*>(command.RawBson()),
		read_prefs ? static_cast<const mongoc_read_prefs_t*>(read_prefs->RawReadPrefs()) : nullptr,
		opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
...
bool MongoCollection::InsertOne(const BsonDocument& document,
								const BsonDocument* opts,
								BsonDocument* reply,
								MongoError* error) {
	if (!impl_ || !impl_->coll) return false;
...
MongoCursor* MongoCollection::FindWithOpts(const BsonDocument& filter,
										   const BsonDocument* opts,
										   const MongoReadPrefs* read_prefs) {
	if (!impl_ || !impl_->coll) return nullptr;
...
bool MongoCollection::UpdateOne(const BsonDocument& selector,
...
bool MongoCollection::UpdateMany(const BsonDocument& selector,
								 const BsonDocument& update,
								 const BsonDocument* opts,
								 BsonDocument* reply,
								 MongoError* error) {
...
bool MongoCollection::ReplaceOne(const BsonDocument& selector,
...
bool MongoCollection::DeleteOne(const BsonDocument& selector,
								const BsonDocument* opts,
								BsonDocument* reply,
								MongoError* error) {
	if (!impl_ || !impl_->coll) return false;
...
bool MongoCollection::DeleteMany(const BsonDocument& selector,
								 const BsonDocument* opts,
								 BsonDocument* reply,
								 MongoError* error) {
	if (!impl_ || !impl_->coll) return false;
...
int64_t MongoCollection::CountDocuments(const BsonDocument& filter,
										const BsonDocument* opts,
										const MongoReadPrefs* read_prefs,
										BsonDocument* reply,
										MongoError* error) {
...
bool MongoCollection::CreateIndex(const BsonDocument& keys,
								  const BsonDocument* opts,
								  BsonDocument* reply,
								  MongoError* error) {
	if (!impl_ || !impl_->coll) return false;
	...
	mongoc_index_model_t* model =
		mongoc_index_model_new(static_cast<const bson_t*>(keys.RawBson()),
							   opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr);
...
MongoCursor* MongoDatabase::Aggregate(const BsonDocument& pipeline,
									  const BsonDocument* opts,
									  const MongoReadPrefs* read_prefs) {
	if (!impl_ || !impl_->db) return nullptr;
	mongoc_cursor_t* cursor = mongoc_database_aggregate(
		impl_->db,
		static_cast<const bson_t*>(pipeline.RawBson()),
		opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
		read_prefs ? static_cast<const mongoc_read_prefs_t*>(read_prefs->RawReadPrefs()) : nullptr);
...
MongoChangeStream* MongoDatabase::Watch(const BsonDocument& pipeline, const BsonDocument* opts) {
	if (!impl_ || !impl_->db) return nullptr;
	mongoc_change_stream_t* stream =
		mongoc_database_watch(impl_->db,
							  static_cast<const bson_t*>(pipeline.RawBson()),
							  opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr);
	if (!stream) return nullptr;
	auto* result = MEM_NEW(MongoChangeStream);
...
bool MongoDatabase::DropWithOpts(const BsonDocument* opts, MongoError* error) {
	if (!impl_ || !impl_->db) return false;
	return mongoc_database_drop_with_opts(
		impl_->db,
		opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
		error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}

bool MongoDatabase::ReadCommandWithOpts(const BsonDocument& command,
										const MongoReadPrefs* read_prefs,
										const BsonDocument* opts,
										BsonDocument* reply,
										MongoError* error) {
...
bool MongoDatabase::WriteCommandWithOpts(const BsonDocument& command,
										 const BsonDocument* opts,
										 BsonDocument* reply,
										 MongoError* error) {
	if (!impl_ || !impl_->db) return false;
...
bool MongoDatabase::ReadWriteCommandWithOpts(const BsonDocument& command,
											 const MongoReadPrefs* read_prefs,
											 const BsonDocument* opts,
											 BsonDocument* reply,
											 MongoError* error) {
...
bool MongoDatabase::CommandWithOpts(const BsonDocument& command,
									const MongoReadPrefs* read_prefs,
									const BsonDocument* opts,
									BsonDocument* reply,
									MongoError* error) {
...
void MongoDatabase::SetReadPrefs(const MongoReadPrefs& read_prefs) {
	if (impl_ && impl_->db)
		mongoc_database_set_read_prefs(
			impl_->db, static_cast<const mongoc_read_prefs_t*>(read_prefs.RawReadPrefs()));
}
...
void MongoDatabase::SetWriteConcern(const MongoWriteConcern& write_concern) {
	if (impl_ && impl_->db)
		mongoc_database_set_write_concern(
			impl_->db, static_cast<const mongoc_write_concern_t*>(write_concern.RawWriteConcern()));
}
...
void MongoDatabase::SetReadConcern(const MongoReadConcern& read_concern) {
	if (impl_ && impl_->db)
		mongoc_database_set_read_concern(
			impl_->db, static_cast<const mongoc_read_concern_t*>(read_concern.RawReadConcern()));
}

char** MongoDatabase::GetCollectionNames(MongoError* error) {
	if (!impl_ || !impl_->db) return nullptr;
	return mongoc_database_get_collection_names_with_opts(
		impl_->db, nullptr, error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}
...
bool MongoDatabase::AddUser(const char* username,
							const char* password,
...
bool MongoDatabase::RemoveUser(const char* username, MongoError* error) {
	if (!impl_ || !impl_->db) return false;
	return mongoc_database_remove_user(
		impl_->db, username, error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}

bool MongoDatabase::RemoveAllUsers(MongoError* error) {
	if (!impl_ || !impl_->db) return false;
	return mongoc_database_remove_all_users(
		impl_->db, error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}
...
void MongoCollection::SetReadPrefs(const MongoReadPrefs& read_prefs) {
	if (impl_ && impl_->coll)
		mongoc_collection_set_read_prefs(
			impl_->coll, static_cast<const mongoc_read_prefs_t*>(read_prefs.RawReadPrefs()));
}
...
void MongoCollection::SetReadConcern(const MongoReadConcern& read_concern) {
	if (impl_ && impl_->coll)
		mongoc_collection_set_read_concern(
			impl_->coll, static_cast<const mongoc_read_concern_t*>(read_concern.RawReadConcern()));
}
...
bool MongoCollection::DropWithOpts(const BsonDocument* opts, MongoError* error) {
	if (!impl_ || !impl_->coll) return false;
	return mongoc_collection_drop_with_opts(
		impl_->coll,
		opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
		error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}

bool MongoCollection::DropIndexWithOpts(const char* index_name,
										const BsonDocument* opts,
										MongoError* error) {
	if (!impl_ || !impl_->coll) return false;
...
char* MongoCollection::KeysToIndexString(const BsonDocument& keys) const {
	if (!impl_ || !impl_->coll) return nullptr;
	return mongoc_collection_keys_to_index_string(static_cast<const bson_t*>(keys.RawBson()));
}

MongoCursor* MongoCollection::Aggregate(const BsonDocument& pipeline,
										const BsonDocument* opts,
										const MongoReadPrefs* read_prefs) {
	if (!impl_ || !impl_->coll) return nullptr;
...
bool MongoCollection::InsertMany(const BsonDocument* documents[],
								 size_t count,
								 const BsonDocument* opts,
								 BsonDocument* reply,
								 MongoError* error) {
...
bool MongoCollection::FindAndModify(const BsonDocument& query,
...
MongoChangeStream* MongoCollection::Watch(const BsonDocument& pipeline, const BsonDocument* opts) {
	if (!impl_ || !impl_->coll) return nullptr;
	mongoc_change_stream_t* stream =
		mongoc_collection_watch(impl_->coll,
								static_cast<const bson_t*>(pipeline.RawBson()),
								opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr);
	if (!stream) return nullptr;
	auto* result = MEM_NEW(MongoChangeStream);
...
bool MongoCollection::Rename(const char* new_db,
							 const char* new_name,
							 bool drop_target_before_rename,
							 MongoError* error) {
	if (!impl_ || !impl_->coll) return false;
	return mongoc_collection_rename_with_opts(impl_->coll,
											  new_db,
											  new_name,
...
bool MongoCollection::RenameWithOpts(const char* new_db,
									 const char* new_name,
									 bool drop_target_before_rename,
									 const BsonDocument* opts,
									 MongoError* error) {
...
bool MongoCollection::CreateIndexesWithOpts(const void* const* models,
											size_t n_models,
											const BsonDocument* opts,
											BsonDocument* reply,
											MongoError* error) {

...
class CLOUD_ENGINE_API MongoStream {
	...
	static MongoStream* NewTls(MongoStream* base_stream,
							   const char* host,
							   void* ssl_opts,
							   int client);
	static MongoStream* NewTlsOpenssl(MongoStream* base_stream,
									  const char* host,
									  void* ssl_opts,
									  int client);
	static MongoStream* NewTlsSecureChannel(MongoStream* base_stream,
											const char* host,
											void* ssl_opts,
											int client);
	static MongoStream* NewTlsSecureTransport(MongoStream* base_stream,
											  const char* host,
											  void* ssl_opts,
											  int client);
	...
	// ── TLS operations ───────────────────────────────────────────────
	bool TlsHandshake(const char* host, int32_t timeout_msec, int* events, MongoError* error);
	bool TlsHandshakeBlock(const char* host, int32_t timeout_msec, MongoError* error);

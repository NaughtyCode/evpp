...
BsonDocument::BsonDocument(const uint8_t* data, size_t length) {
	...
	if (!bson_init_static(b, data, length)) bson_init(b);
}
...
BsonDocument::BsonDocument(BsonDocument&& other) noexcept {
	bson_steal(static_cast<bson_t*>(RawBson()), static_cast<bson_t*>(other.RawBson()));
}
...
BsonDocument BsonDocument::Copy() const {
	BsonDocument result;
	bson_copy_to(static_cast<const bson_t*>(RawBson()), static_cast<bson_t*>(result.RawBson()));
	return result;
}

bool BsonDocument::CopyTo(BsonDocument& dst) const {
	bson_copy_to(static_cast<const bson_t*>(RawBson()), static_cast<bson_t*>(dst.RawBson()));
	return true;
}
...
bool BsonDocument::CopyToExcludingNoinitVa(BsonDocument& dst,
...
bool BsonDocument::AppendUtf8(const char* key, std::string_view value) {
	...
	return bson_append_utf8(
		static_cast<bson_t*>(RawBson()), key, -1, value.data(), static_cast<int32_t>(len));
}

bool BsonDocument::AppendDocument(const char* key, const BsonDocument& subdoc) {
	return bson_append_document(
		static_cast<bson_t*>(RawBson()), key, -1, static_cast<const bson_t*>(subdoc.RawBson()));
}

bool BsonDocument::AppendArray(const char* key, const BsonDocument& array) {
	return bson_append_array(
		static_cast<bson_t*>(RawBson()), key, -1, static_cast<const bson_t*>(array.RawBson()));
}

bool BsonDocument::AppendBinary(const char* key,
								int subtype,
								const uint8_t* data,
								uint32_t length) {
	return bson_append_binary(static_cast<bson_t*>(RawBson()),
							  key,
							  -1,
							  static_cast<bson_subtype_t>(subtype),
							  data,
							  length);
}
...
bool BsonDocument::AppendOid(const char* key, const MongoOid& oid) {
	return bson_append_oid(
		static_cast<bson_t*>(RawBson()), key, -1, reinterpret_cast<const bson_oid_t*>(oid.data()));
}
...
bool BsonDocument::AppendCodeWithScope(const char* key,
									   const char* javascript,
									   const BsonDocument& scope) {
	return bson_append_code_with_scope(static_cast<bson_t*>(RawBson()),
									   key,
									   -1,
									   javascript,
...
bool BsonDocument::AppendRegexWLen(const char* key,
								   int keylen,
								   const char* regex,
								   const char* options) {
	return bson_append_regex_w_len(
		static_cast<bson_t*>(RawBson()), key, keylen, regex, -1, options);
}
...
bool BsonDocument::AppendDBPointer(const char* key, const char* collection, const MongoOid& oid) {
	return bson_append_dbpointer(static_cast<bson_t*>(RawBson()),
								 key,
								 -1,
								 collection,
...
bool BsonDocument::AppendTimeval(const char* key, const void* tv) {
	return bson_append_timeval(static_cast<bson_t*>(RawBson()),
							   key,
							   -1,
...
bool BsonDocument::AppendDecimal128(const char* key, const MongoDecimal128& value) {
	return bson_append_decimal128(static_cast<bson_t*>(RawBson()),
								  key,
								  -1,
...
bool BsonDocument::AppendValue(const char* key, const void* bson_value) {
	return bson_append_value(
		static_cast<bson_t*>(RawBson()), key, -1, static_cast<const bson_value_t*>(bson_value));
}

bool BsonDocument::AppendIter(const char* key, const BsonIter& iter) {
	return bson_append_iter(
		static_cast<bson_t*>(RawBson()), key, -1, static_cast<const bson_iter_t*>(iter.RawIter()));
}

bool BsonDocument::AppendBinaryUninit(const char* key,
									  int subtype,
									  uint8_t** data_out,
									  uint32_t len) {
	return bson_append_binary_uninit(static_cast<bson_t*>(RawBson()),
									 key,
									 -1,
									 static_cast<bson_subtype_t>(subtype),
									 data_out,
...
bool BsonDocument::AppendArrayFromVector(const char* key, const BsonIter& iter) {
	return bson_append_array_from_vector(
		static_cast<bson_t*>(RawBson()), key, -1, static_cast<const bson_iter_t*>(iter.RawIter()));
}

bool BsonDocument::AppendDocumentBegin(const char* key, BsonDocument* subdoc) {
	return bson_append_document_begin(
		static_cast<bson_t*>(RawBson()), key, -1, static_cast<bson_t*>(subdoc->RawBson()));
}
...
bool BsonDocument::AppendArrayBegin(const char* key, BsonDocument* array) {
	return bson_append_array_begin(
		static_cast<bson_t*>(RawBson()), key, -1, static_cast<bson_t*>(array->RawBson()));
}

bool BsonDocument::AppendArrayUnsafeBegin(const char* key, BsonDocument* child) {
	return bson_append_array_unsafe_begin(
		static_cast<bson_t*>(RawBson()), key, -1, static_cast<bson_t*>(child->RawBson()));
}
...
bool BsonDocument::Concat(const BsonDocument& src) {
	return bson_concat(static_cast<bson_t*>(RawBson()), static_cast<const bson_t*>(src.RawBson()));
}
...
char* BsonDocument::AsJsonWithOpts(size_t* length, const void* opts) const {
	return bson_as_json_with_opts(
		static_cast<const bson_t*>(RawBson()), length, static_cast<const bson_json_opts_t*>(opts));
}

char* BsonDocument::ArrayAsCanonicalExtendedJson(const BsonDocument& array, size_t* length) {
	return bson_array_as_canonical_extended_json(static_cast<const bson_t*>(array.RawBson()),
												 length);
}

char* BsonDocument::ArrayAsRelaxedExtendedJson(const BsonDocument& array, size_t* length) {
	return bson_array_as_relaxed_extended_json(static_cast<const bson_t*>(array.RawBson()), length);
}

char* BsonDocument::ArrayAsLegacyExtendedJson(const BsonDocument& array, size_t* length) {
	return bson_array_as_legacy_extended_json(static_cast<const bson_t*>(array.RawBson()), length);
}
...
BsonDocument BsonDocument::NewFromBuffer(uint8_t** buf,
										 size_t* buf_len,
										 void* realloc_func,
...
void BsonDocument::Steal(BsonDocument& dst, BsonDocument& src) {
	...
	bson_steal(static_cast<bson_t*>(dst.RawBson()), static_cast<bson_t*>(src.RawBson()));
}

void BsonDocument::DestroyWithSteal(uint8_t** data,
									uint32_t* length,
									bool* reached_eof,
									void* raw_bson) {
...
bool BsonDocument::AppendArrayBuilderBegin(const char* key, void** builder_out) {
	return bson_append_array_builder_begin(static_cast<bson_t*>(RawBson()),
										   key,
										   -1,
...
bool BsonDocument::Validate(MongoError* error) const {
	bson_error_t err;
	bool ok =
		bson_validate_with_error(static_cast<const bson_t*>(RawBson()), BSON_VALIDATE_NONE, &err);
	if (!ok && error) {
...
bool BsonDocument::InitFromJson(const char* json, int64_t len, MongoError* error) {
	return bson_init_from_json(static_cast<bson_t*>(RawBson()),
							   json,
							   len,
							   error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}

void* BsonDocument::RawBson() {
	return static_cast<void*>(storage_);
}
const void* BsonDocument::RawBson() const {
...
BsonIter::BsonIter(const BsonDocument& doc) {
	bson_iter_init(static_cast<bson_iter_t*>(RawIter()), static_cast<const bson_t*>(doc.RawBson()));
}
...
void BsonIter::AsCodeWithScope(uint32_t* code_length,
							   const char** code,
							   BsonDocument* scope) const {
	if (!code_length) return;
	uint32_t scope_len;
	const uint8_t* scope_data;
	bson_iter_codewscope(
		static_cast<const bson_iter_t*>(RawIter()), code_length, &scope_len, &scope_data);
	if (code && scope_data) {
...
bool BsonIter::FindDescendant(const char* dotkey, BsonIter* descendant) {
	...
	return bson_iter_find_descendant(static_cast<bson_iter_t*>(RawIter()),
									 dotkey,
...
bool BsonIter::InitFind(const BsonDocument& doc, const char* key) {
	return bson_iter_init_find(
		static_cast<bson_iter_t*>(RawIter()), static_cast<const bson_t*>(doc.RawBson()), key);
}

bool BsonIter::InitFindCase(const BsonDocument& doc, const char* key) {
	return bson_iter_init_find_case(
		static_cast<bson_iter_t*>(RawIter()), static_cast<const bson_t*>(doc.RawBson()), key);
}
...
bool BsonIter::InitFromDataAtOffset(const uint8_t* data,
									size_t length,
									uint32_t offset,
									uint32_t keylen) {
	return bson_iter_init_from_data_at_offset(
		static_cast<bson_iter_t*>(RawIter()), data, length, offset, keylen);
}
...
int BsonIter::BinarySubtype() const {
	return static_cast<int>(bson_iter_binary_subtype(static_cast<const bson_iter_t*>(RawIter())));
}

bool BsonIter::BinaryEqual(const BsonIter& a, const BsonIter& b) {
	return bson_iter_binary_equal(static_cast<const bson_iter_t*>(a.RawIter()),
								  static_cast<const bson_iter_t*>(b.RawIter()));
}

void BsonIter::AsTimeval(void* tv) const {
	bson_iter_timeval(static_cast<const bson_iter_t*>(RawIter()), static_cast<struct timeval*>(tv));
}
void BsonIter::AsDBPointer(uint32_t* collection_len,
...
bool BsonIter::VisitAll(const void* visitor, void* data) {
	return bson_iter_visit_all(
		static_cast<bson_iter_t*>(RawIter()), static_cast<const bson_visitor_t*>(visitor), data);
}
...
void BsonIter::OverwriteTimestamp(uint32_t timestamp, uint32_t increment) {
	bson_iter_overwrite_timestamp(static_cast<bson_iter_t*>(RawIter()), timestamp, increment);
}
...
void* BsonIter::RawIter() {
	return static_cast<void*>(storage_);
}

const void* BsonIter::RawIter() const {
	return static_cast<const void*>(storage_);
}
...
bool MongoDecimal128::FromStringLen(const char* str, int len) {
	return bson_decimal128_from_string_w_len(str, len, reinterpret_cast<bson_decimal128_t*>(this));
}
...
bool BsonArrayBuilder::AppendDouble(double value) {
	return bson_array_builder_append_double(static_cast<bson_array_builder_t*>(ptr_), value);
}

bool BsonArrayBuilder::AppendUtf8(const char* value) {
	return bson_array_builder_append_utf8(static_cast<bson_array_builder_t*>(ptr_), value, -1);
}

bool BsonArrayBuilder::AppendInt32(int32_t value) {
	return bson_array_builder_append_int32(static_cast<bson_array_builder_t*>(ptr_), value);
}

bool BsonArrayBuilder::AppendInt64(int64_t value) {
	return bson_array_builder_append_int64(static_cast<bson_array_builder_t*>(ptr_), value);
}

bool BsonArrayBuilder::AppendBool(bool value) {
	return bson_array_builder_append_bool(static_cast<bson_array_builder_t*>(ptr_), value);
}

bool BsonArrayBuilder::AppendOid(const MongoOid& oid) {
	return bson_array_builder_append_oid(static_cast<bson_array_builder_t*>(ptr_),
										 reinterpret_cast<const bson_oid_t*>(oid.data()));
}

bool BsonArrayBuilder::AppendNull() {
	return bson_array_builder_append_null(static_cast<bson_array_builder_t*>(ptr_));
}

bool BsonArrayBuilder::AppendDateTime(int64_t msec_since_epoch) {
	return bson_array_builder_append_date_time(static_cast<bson_array_builder_t*>(ptr_),
											   msec_since_epoch);
}
...
bool BsonArrayBuilder::AppendDocument(const BsonDocument& doc) {
	return bson_array_builder_append_document(static_cast<bson_array_builder_t*>(ptr_),
											  static_cast<const bson_t*>(doc.RawBson()));
}

bool BsonArrayBuilder::AppendArray(const BsonDocument& array) {
	return bson_array_builder_append_array(static_cast<bson_array_builder_t*>(ptr_),
										   static_cast<const bson_t*>(array.RawBson()));
}

bool BsonArrayBuilder::AppendBinary(int subtype, const uint8_t* data, uint32_t length) {
	return bson_array_builder_append_binary(static_cast<bson_array_builder_t*>(ptr_),
											static_cast<bson_subtype_t>(subtype),
											data,
											length);
}
...
bool BsonArrayBuilder::AppendCode(const char* javascript) {
	return bson_array_builder_append_code(static_cast<bson_array_builder_t*>(ptr_), javascript);
}

bool BsonArrayBuilder::AppendCodeWithScope(const char* javascript, const BsonDocument& scope) {
	return bson_array_builder_append_code_with_scope(static_cast<bson_array_builder_t*>(ptr_),
													 javascript,
													 static_cast<const bson_t*>(scope.RawBson()));
}

bool BsonArrayBuilder::AppendIter(const BsonIter& iter) {
	return bson_array_builder_append_iter(static_cast<bson_array_builder_t*>(ptr_),
										  static_cast<const bson_iter_t*>(iter.RawIter()));
}

bool BsonArrayBuilder::AppendValue(const void* bson_value) {
	return bson_array_builder_append_value(static_cast<bson_array_builder_t*>(ptr_),
...
bool BsonArrayBuilder::AppendSymbol(const char* value) {
	return bson_array_builder_append_symbol(static_cast<bson_array_builder_t*>(ptr_), value, -1);
}

bool BsonArrayBuilder::AppendDBPointer(const char* collection, const MongoOid& oid) {
	return bson_array_builder_append_dbpointer(static_cast<bson_array_builder_t*>(ptr_),
											   collection,
											   reinterpret_cast<const bson_oid_t*>(oid.data()));
}
...
bool BsonArrayBuilder::AppendTimeval(const void* tv) {
	return bson_array_builder_append_timeval(
...
bool BsonArrayBuilder::AppendDecimal128(const MongoDecimal128& value) {
	return bson_array_builder_append_decimal128(static_cast<bson_array_builder_t*>(ptr_),
												reinterpret_cast<const bson_decimal128_t*>(&value));
}

bool BsonArrayBuilder::AppendArrayFromVector(const BsonIter& iter) {
	return bson_array_builder_append_array_from_vector(
		static_cast<bson_array_builder_t*>(ptr_), static_cast<const bson_iter_t*>(iter.RawIter()));
}

bool BsonArrayBuilder::AppendBinaryUninit(int subtype, uint8_t** data_out, uint32_t len) {
	return bson_array_builder_append_binary_uninit(static_cast<bson_array_builder_t*>(ptr_),
												   static_cast<bson_subtype_t>(subtype),
												   data_out,
												   len);
}
...
bool BsonArrayBuilder::AppendDocumentBegin(BsonDocument* subdoc) {
	return bson_array_builder_append_document_begin(static_cast<bson_array_builder_t*>(ptr_),
													static_cast<bson_t*>(subdoc->RawBson()));
}

bool BsonArrayBuilder::AppendDocumentEnd(BsonArrayBuilder* builder, BsonDocument* subdoc) {
	return bson_array_builder_append_document_end(static_cast<bson_array_builder_t*>(builder->ptr_),
												  static_cast<bson_t*>(subdoc->RawBson()));
}
...
bool BsonArrayBuilder::Build(BsonDocument* out) {
	return bson_array_builder_build(static_cast<bson_array_builder_t*>(ptr_),
									static_cast<bson_t*>(out->RawBson()));
}

void* BsonArrayBuilder::Raw() {
	return ptr_;
}

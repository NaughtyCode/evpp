...
bool MongoFindAndModifyOpts::SetSort(const BsonDocument& sort) {
	return impl_ && impl_->opts &&
		   mongoc_find_and_modify_opts_set_sort(impl_->opts,
												static_cast<const bson_t*>(sort.RawBson()));
}

void MongoFindAndModifyOpts::GetSort(BsonDocument& out) const {
	if (impl_ && impl_->opts)
		mongoc_find_and_modify_opts_get_sort(impl_->opts, static_cast<bson_t*>(out.RawBson()));
}

bool MongoFindAndModifyOpts::SetUpdate(const BsonDocument& update) {
	return impl_ && impl_->opts &&
		   mongoc_find_and_modify_opts_set_update(impl_->opts,
												  static_cast<const bson_t*>(update.RawBson()));
}

void MongoFindAndModifyOpts::GetUpdate(BsonDocument& out) const {
	if (impl_ && impl_->opts)
		mongoc_find_and_modify_opts_get_update(impl_->opts, static_cast<bson_t*>(out.RawBson()));
}

bool MongoFindAndModifyOpts::SetFields(const BsonDocument& fields) {
	return impl_ && impl_->opts &&
		   mongoc_find_and_modify_opts_set_fields(impl_->opts,
												  static_cast<const bson_t*>(fields.RawBson()));
}

void MongoFindAndModifyOpts::GetFields(BsonDocument& out) const {
	if (impl_ && impl_->opts)
		mongoc_find_and_modify_opts_get_fields(impl_->opts, static_cast<bson_t*>(out.RawBson()));
}
...
bool MongoFindAndModifyOpts::SetBypassDocumentValidation(bool bypass) {
	return impl_ && impl_->opts &&
		   mongoc_find_and_modify_opts_set_bypass_document_validation(impl_->opts, bypass);
}

bool MongoFindAndModifyOpts::GetBypassDocumentValidation() const {
	return impl_ && impl_->opts &&
		   mongoc_find_and_modify_opts_get_bypass_document_validation(impl_->opts);
}

bool MongoFindAndModifyOpts::SetMaxTimeMs(uint32_t max_time_ms) {
	return impl_ && impl_->opts &&
		   mongoc_find_and_modify_opts_set_max_time_ms(impl_->opts, max_time_ms);
}
...
bool MongoFindAndModifyOpts::Append(const BsonDocument& extra) {
	return impl_ && impl_->opts &&
		   mongoc_find_and_modify_opts_append(impl_->opts,
											  static_cast<const bson_t*>(extra.RawBson()));
}

void MongoFindAndModifyOpts::GetExtra(BsonDocument& out) const {
	if (impl_ && impl_->opts)
		mongoc_find_and_modify_opts_get_extra(impl_->opts, static_cast<bson_t*>(out.RawBson()));
}

...
MongoStream* MongoStream::NewGridFs(void* gridfs_file) {
    auto* result = new MongoStream();
    result->impl_->stream = mongoc_stream_gridfs_new(
        static_cast<mongoc_gridfs_file_t*>(gridfs_file));
    if (!result->impl_->stream) { delete result; return nullptr; }
    return result;
}
...
MongoStream* MongoStream::NewSocket(void* socket) {
    auto* result = new MongoStream();
    result->impl_->stream = mongoc_stream_socket_new(
        static_cast<mongoc_socket_t*>(socket));
    if (!result->impl_->stream) { delete result; return nullptr; }
    return result;
}

MongoStream* MongoStream::NewTls(MongoStream* base_stream, const char* host,
...
MongoStream* MongoStream::NewTlsOpenssl(MongoStream* base_stream, const char* host,
                                          void* ssl_opts, int client) {
...
ssize_t MongoStream::Writev(void* iov, size_t iovcnt, int32_t timeout_msec) {
    return impl_ && impl_->stream
        ? mongoc_stream_writev(impl_->stream, static_cast<mongoc_iovec_t*>(iov),
                                iovcnt, timeout_msec)
        : -1;
}
...
ssize_t MongoStream::Write(void* buf, size_t count, int32_t timeout_msec) {
    return impl_ && impl_->stream
        ? mongoc_stream_write(impl_->stream, buf, count, timeout_msec) : -1;
}

ssize_t MongoStream::Readv(void* iov, size_t iovcnt, size_t min_bytes,
                             int32_t timeout_msec) {
    return impl_ && impl_->stream
        ? mongoc_stream_readv(impl_->stream, static_cast<mongoc_iovec_t*>(iov),
                               iovcnt, min_bytes, timeout_msec)
        : -1;
}

ssize_t MongoStream::Read(void* buf, size_t count, size_t min_bytes,
                            int32_t timeout_msec) {
    return impl_ && impl_->stream
...
int MongoStream::SetSockopt(int level, int optname, void* optval, int optlen) {
    return impl_ && impl_->stream
        ? mongoc_stream_setsockopt(impl_->stream, level, optname, optval,
                                    static_cast<mongoc_socklen_t>(optlen))
        : -1;
}
...
bool MongoStream::TlsHandshake(const char* host, int32_t timeout_msec,
                                int* events, MongoError* error) {
    return impl_ && impl_->stream && mongoc_stream_tls_handshake(
        impl_->stream, host, timeout_msec, events,
        error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}

bool MongoStream::TlsHandshakeBlock(const char* host, int32_t timeout_msec,
                                      MongoError* error) {
    return impl_ && impl_->stream && mongoc_stream_tls_handshake_block(
        impl_->stream, host, timeout_msec,
        error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}
...
int MongoStream::GetFileFd() {
    return impl_ && impl_->stream
        ? mongoc_stream_file_get_fd(
              reinterpret_cast<mongoc_stream_file_t*>(impl_->stream))
        : -1;
}
...
ssize_t MongoStream::Poll(void* streams, size_t nstreams, int32_t timeout_ms) {
    return mongoc_stream_poll(static_cast<mongoc_stream_poll_t*>(streams),
                               nstreams, timeout_ms);
}

void* MongoStream::Raw() { return impl_ ? impl_->stream : nullptr; }

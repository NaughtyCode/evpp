...
class ENGINE_API MongoGridFs {
	...
	MongoGridFsFile* FindOneWithOpts(const BsonDocument& filter,
...
class ENGINE_API MongoGridFsBucket {
	...
	// Upload
	void* OpenUploadStream(const char* filename,
						   const BsonDocument* opts,
						   void* file_id_out,
						   MongoError* error);
	void* OpenUploadStreamWithId(const void* file_id,
								 const char* filename,
								 const BsonDocument* opts,
								 MongoError* error);
	bool UploadFromStream(const char* filename,
						  void* source_stream,
						  const BsonDocument* opts,
						  void* file_id_out,
						  MongoError* error);
	bool UploadFromStreamWithId(const void* file_id,
								const char* filename,
								void* source_stream,
								const BsonDocument* opts,
								MongoError* error);

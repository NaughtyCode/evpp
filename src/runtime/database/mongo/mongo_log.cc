...
MongoStructuredLogEntry::MongoStructuredLogEntry(const void* raw_entry)
    : entry_(raw_entry) {}
...
MongoStructuredLogComponent MongoStructuredLogEntry::GetComponent() const {
    return static_cast<MongoStructuredLogComponent>(
...
void MongoStructuredLogOpts::SetHandler(LogFunc handler) {
    ...
    if (impl_->handler) {
        mongoc_structured_log_opts_set_handler(impl_->opts, structured_log_trampoline,
            &impl_->handler);
    } else {
...
bool MongoStructuredLogOpts::SetMaxLevelForComponent(MongoStructuredLogComponent component,
    ...
    return impl_ && impl_->opts && mongoc_structured_log_opts_set_max_level_for_component(
        impl_->opts,
...
bool MongoStructuredLogOpts::SetMaxLevelForAllComponents(MongoStructuredLogLevel level) {
    return impl_ && impl_->opts && mongoc_structured_log_opts_set_max_level_for_all_components(
...
MongoStructuredLogLevel MongoStructuredLogOpts::GetMaxLevelForComponent(
    ...
    return static_cast<MongoStructuredLogLevel>(
        mongoc_structured_log_opts_get_max_level_for_component(impl_->opts,
            static_cast<mongoc_structured_log_component_t>(component)));
}

size_t MongoStructuredLogOpts::GetMaxDocumentLength() const {
    return impl_ && impl_->opts
        ? mongoc_structured_log_opts_get_max_document_length(impl_->opts) : 0;
}

bool MongoStructuredLogOpts::SetMaxDocumentLength(size_t max_len) {
    return impl_ && impl_->opts && mongoc_structured_log_opts_set_max_document_length(
        impl_->opts, max_len);
}

bool MongoStructuredLogOpts::SetMaxDocumentLengthFromEnv() {
    return impl_ && impl_->opts && mongoc_structured_log_opts_set_max_document_length_from_env(
        impl_->opts);
}

const char* MongoStructuredLogOpts::GetLevelName(MongoStructuredLogLevel level) {
    return mongoc_structured_log_get_level_name(
        static_cast<mongoc_structured_log_level_t>(level));
}
...
bool MongoStructuredLogOpts::GetNamedComponent(const char* name,
                                                MongoStructuredLogComponent* out) {
...
const void* MongoStructuredLogOpts::Raw() const { return impl_ ? impl_->opts : nullptr; }
void* MongoStructuredLogOpts::Raw() { return impl_ ? impl_->opts : nullptr; }

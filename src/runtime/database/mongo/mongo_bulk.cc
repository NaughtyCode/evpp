...
MongoBulkOperation::~MongoBulkOperation() { Destroy(); }
...
void MongoBulkOperation::RemoveOne(const BsonDocument& selector) {
    if (impl_ && impl_->bulk)
        mongoc_bulk_operation_remove_one(impl_->bulk, static_cast<const bson_t*>(selector.RawBson()));
}

void MongoBulkOperation::ReplaceOne(const BsonDocument& selector, const BsonDocument& document, bool upsert) {
    if (impl_ && impl_->bulk)
        mongoc_bulk_operation_replace_one(impl_->bulk, static_cast<const bson_t*>(selector.RawBson()),
                                           static_cast<const bson_t*>(document.RawBson()), upsert);
}

void MongoBulkOperation::Update(const BsonDocument& selector, const BsonDocument& document, bool upsert) {
    if (impl_ && impl_->bulk)
        mongoc_bulk_operation_update(impl_->bulk, static_cast<const bson_t*>(selector.RawBson()),
                                      static_cast<const bson_t*>(document.RawBson()), upsert);
}

void MongoBulkOperation::UpdateOne(const BsonDocument& selector, const BsonDocument& document, bool upsert) {
    if (impl_ && impl_->bulk)
        mongoc_bulk_operation_update_one(impl_->bulk, static_cast<const bson_t*>(selector.RawBson()),
                                          static_cast<const bson_t*>(document.RawBson()), upsert);
}

bool MongoBulkOperation::InsertWithOpts(const BsonDocument& document, const BsonDocument* opts, MongoError* error) {
    if (!impl_ || !impl_->bulk) return false;
    return mongoc_bulk_operation_insert_with_opts(impl_->bulk,
...
bool MongoBulkOperation::RemoveOneWithOpts(const BsonDocument& selector, const BsonDocument* opts, MongoError* error) {
    if (!impl_ || !impl_->bulk) return false;
    return mongoc_bulk_operation_remove_one_with_opts(impl_->bulk,
        static_cast<const bson_t*>(selector.RawBson()),
        opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
        error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}

bool MongoBulkOperation::RemoveManyWithOpts(const BsonDocument& selector, const BsonDocument* opts, MongoError* error) {
    if (!impl_ || !impl_->bulk) return false;
    return mongoc_bulk_operation_remove_many_with_opts(impl_->bulk,
        static_cast<const bson_t*>(selector.RawBson()),
        opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
        error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}

bool MongoBulkOperation::ReplaceOneWithOpts(const BsonDocument& selector, const BsonDocument& document,
                                             const BsonDocument* opts, MongoError* error) {
    if (!impl_ || !impl_->bulk) return false;
    return mongoc_bulk_operation_replace_one_with_opts(impl_->bulk,
        static_cast<const bson_t*>(selector.RawBson()),
        static_cast<const bson_t*>(document.RawBson()),
        opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
        error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
...
bool MongoBulkOperation::UpdateOneWithOpts(const BsonDocument& selector, const BsonDocument& document,
                                            const BsonDocument* opts, MongoError* error) {
    if (!impl_ || !impl_->bulk) return false;
    return mongoc_bulk_operation_update_one_with_opts(impl_->bulk,
        static_cast<const bson_t*>(selector.RawBson()),
        static_cast<const bson_t*>(document.RawBson()),
        opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
        error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
...
bool MongoBulkOperation::UpdateManyWithOpts(const BsonDocument& selector, const BsonDocument& document,
                                             const BsonDocument* opts, MongoError* error) {
    if (!impl_ || !impl_->bulk) return false;
    return mongoc_bulk_operation_update_many_with_opts(impl_->bulk,
        static_cast<const bson_t*>(selector.RawBson()),
        static_cast<const bson_t*>(document.RawBson()),
        opts ? static_cast<const bson_t*>(opts->RawBson()) : nullptr,
        error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
...
void MongoBulkOperation::SetWriteConcern(const MongoWriteConcern& write_concern) {
    if (impl_ && impl_->bulk)
        mongoc_bulk_operation_set_write_concern(impl_->bulk,
...
void MongoBulkOperation::SetServerId(uint32_t server_id) {
    if (impl_ && impl_->bulk)
        mongoc_bulk_operation_set_server_id(impl_->bulk, server_id);
}

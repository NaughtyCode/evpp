...
BsonJsonReader BsonJsonReader::NewFromFile(const char* filename, MongoError* error) {
	BsonJsonReader r;
	r.impl_->reader = bson_json_reader_new_from_file(
		filename, error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
	return r;
}
...
BsonJsonReader BsonJsonReader::NewFromData(const uint8_t* data, size_t length) {
	...
	if (r.impl_->reader) bson_json_data_reader_ingest(r.impl_->reader, data, length);
	return r;
}

BsonJsonReader BsonJsonReader::New(
...
BsonReader BsonReader::NewFromFile(const char* path, MongoError* error) {
	BsonReader r;
	r.impl_->reader = bson_reader_new_from_file(
		path, error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
	return r;
}
...
BsonReader BsonReader::NewFromHandle(void* handle, void* read_func, void* destroy_func) {
	BsonReader r;
	r.impl_->reader =
		bson_reader_new_from_handle(handle,
...
void BsonReader::SetReadFunc(void* func) {
	if (impl_ && impl_->reader)
		bson_reader_set_read_func(impl_->reader, reinterpret_cast<bson_reader_read_func_t>(func));
}
...
void BsonReader::Reset() {
	if (impl_ && impl_->reader) bson_reader_reset(impl_->reader);
}
...
BsonJsonOpts::BsonJsonOpts(BsonJsonMode mode, int32_t max_len) : impl_(std::make_unique<Impl>()) {
...
void BsonJsonOpts::SetOutermostArray(bool is_outermost_array) {
	if (impl_ && impl_->opts) bson_json_opts_set_outermost_array(impl_->opts, is_outermost_array);
}

void* BsonJsonOpts::Raw() {
...
const void* BsonValue::Raw() const {
	return storage_;
}

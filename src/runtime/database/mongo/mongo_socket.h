...
public:
    ...
    ssize_t Recv(void* buf, size_t buflen, int flags, int64_t expire_at);
    ssize_t Send(const void* buf, size_t buflen, int64_t expire_at);
    ssize_t Sendv(MongoIovec* iov, size_t iovcnt, int64_t expire_at);
    ...
    static ssize_t Poll(MongoSocketPollFd* sds, size_t nsds, int32_t timeout);

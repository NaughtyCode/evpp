...
void* MongoServerDescription::Raw() const {
	return sd_;
}
...
MongoServerDescription** MongoTopologyDescription::GetServers(size_t* n) const {
	...
	auto** result =
		static_cast<MongoServerDescription**>(bson_malloc(count * sizeof(MongoServerDescription*)));
	if (!result) {
...
void* MongoTopologyDescription::Raw() const {
	return td_;
}

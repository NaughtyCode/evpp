...
MongoUri MongoUri::NewWithError(const char* uri_string, MongoError* error) {
	MongoUri result;
	result.impl_->uri = mongoc_uri_new_with_error(
		uri_string, error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
	return result;
}
...
void MongoUri::SetReadPrefs(const MongoReadPrefs& read_prefs) {
	if (impl_ && impl_->uri)
		mongoc_uri_set_read_prefs_t(
			impl_->uri, static_cast<const mongoc_read_prefs_t*>(read_prefs.RawReadPrefs()));
}
...
void MongoUri::SetReadConcern(const MongoReadConcern& read_concern) {
	if (impl_ && impl_->uri)
		mongoc_uri_set_read_concern(
			impl_->uri, static_cast<const mongoc_read_concern_t*>(read_concern.RawReadConcern()));
}
...
bool MongoUri::OptionIsInt32(const char* key) {
	return mongoc_uri_option_is_int32(key);
}
bool MongoUri::OptionIsInt64(const char* key) {
	return mongoc_uri_option_is_int64(key);
}
bool MongoUri::OptionIsBool(const char* key) {
	return mongoc_uri_option_is_bool(key);
}
bool MongoUri::OptionIsUtf8(const char* key) {
	return mongoc_uri_option_is_utf8(key);
}

// ── Generic option getters ────────────────────────────────────────────

int32_t MongoUri::GetOptionAsInt32(const char* option, int32_t fallback) const {
	return impl_ && impl_->uri ? mongoc_uri_get_option_as_int32(impl_->uri, option, fallback)
							   : fallback;
}

int64_t MongoUri::GetOptionAsInt64(const char* option, int64_t fallback) const {
	return impl_ && impl_->uri ? mongoc_uri_get_option_as_int64(impl_->uri, option, fallback)
							   : fallback;
}

bool MongoUri::GetOptionAsBool(const char* option, bool fallback) const {
	return impl_ && impl_->uri ? mongoc_uri_get_option_as_bool(impl_->uri, option, fallback)
							   : fallback;
}

const char* MongoUri::GetOptionAsUtf8(const char* option, const char* fallback) const {
	return impl_ && impl_->uri ? mongoc_uri_get_option_as_utf8(impl_->uri, option, fallback)
							   : fallback;
}

...
MongoTransactionOpts::MongoTransactionOpts(MongoTransactionOpts&& other) noexcept
...
void MongoTransactionOpts::SetMaxCommitTimeMs(int64_t max_commit_time_ms) {
	if (impl_ && impl_->opts)
		mongoc_transaction_opts_set_max_commit_time_ms(impl_->opts, max_commit_time_ms);
}
...
void MongoTransactionOpts::SetReadConcern(const MongoReadConcern& read_concern) {
	if (impl_ && impl_->opts)
		mongoc_transaction_opts_set_read_concern(
			impl_->opts, static_cast<const mongoc_read_concern_t*>(read_concern.RawReadConcern()));
}
...
void MongoTransactionOpts::SetReadPrefs(const MongoReadPrefs& read_prefs) {
	if (impl_ && impl_->opts)
		mongoc_transaction_opts_set_read_prefs(
			impl_->opts, static_cast<const mongoc_read_prefs_t*>(read_prefs.RawReadPrefs()));
}
...
MongoSessionOpts::MongoSessionOpts(MongoSessionOpts&& other) noexcept
	: impl_(std::move(other.impl_)) {
}
...
void MongoSessionOpts::SetCausalConsistency(bool causal_consistency) {
	if (impl_ && impl_->opts)
		mongoc_session_opts_set_causal_consistency(impl_->opts, causal_consistency);
}
...
const void* MongoSessionOpts::GetDefaultTransactionOptsRaw() const {
	return impl_ && impl_->opts ? mongoc_session_opts_get_default_transaction_opts(impl_->opts)
								: nullptr;
}
...
bool MongoSession::StartTransaction(const MongoTransactionOpts* opts, MongoError* error) {
	...
	return mongoc_client_session_start_transaction(
		impl_->session,
...
bool MongoSession::CommitTransaction(BsonDocument* reply, MongoError* error) {
	if (!impl_ || !impl_->session) return false;
	return mongoc_client_session_commit_transaction(
		impl_->session,
...
bool MongoSession::AbortTransaction(MongoError* error) {
	if (!impl_ || !impl_->session) return false;
	return mongoc_client_session_abort_transaction(
		impl_->session, error ? static_cast<bson_error_t*>(error->RawError()) : nullptr);
}
...
MongoSession::TransactionState MongoSession::GetTransactionState() const {
	...
	return static_cast<TransactionState>(
		mongoc_client_session_get_transaction_state(impl_->session));
}

const void* MongoSession::GetClusterTimeRaw() const {
	return impl_ && impl_->session ? mongoc_client_session_get_cluster_time(impl_->session)
								   : nullptr;
}
...
void MongoSession::AdvanceClusterTime(const BsonDocument& cluster_time) {
	if (impl_ && impl_->session)
		mongoc_client_session_advance_cluster_time(
			impl_->session, static_cast<const bson_t*>(cluster_time.RawBson()));
}
...
const void* MongoSession::GetTransactionOptsRaw() const {
	return impl_ && impl_->session ? mongoc_session_opts_get_transaction_opts(impl_->session)
								   : nullptr;
}

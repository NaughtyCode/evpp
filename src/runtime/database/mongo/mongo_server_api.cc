...
MongoServerApi MongoServerApi::New(Version version) {
	...
	result.impl_->api = mongoc_server_api_new(static_cast<mongoc_server_api_version_t>(version));
	return result;
}

const char* MongoServerApi::VersionToString(Version version) {
	return mongoc_server_api_version_to_string(static_cast<mongoc_server_api_version_t>(version));
}

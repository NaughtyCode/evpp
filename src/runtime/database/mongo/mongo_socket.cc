...
ssize_t MongoSocket::Receive(void* buf, size_t buflen, int flags, int64_t expire_at) {
	return impl_ && impl_->sock ? mongoc_socket_recv(impl_->sock, buf, buflen, flags, expire_at)
								: -1;
}

ssize_t MongoSocket::SendData(const void* buf, size_t buflen, int64_t expire_at) {
	return impl_ && impl_->sock ? mongoc_socket_send(impl_->sock, buf, buflen, expire_at) : -1;
}

ssize_t MongoSocket::SendvData(MongoIovec* iov, size_t iovcnt, int64_t expire_at) {
	return impl_ && impl_->sock
			   ? mongoc_socket_sendv(
					 impl_->sock, reinterpret_cast<mongoc_iovec_t*>(iov), iovcnt, expire_at)
			   : -1;
}

int MongoSocket::SetSockOpt(int level, int optname, const void* optval, int optlen) {
	return impl_ && impl_->sock
			   ? mongoc_socket_setsockopt(impl_->sock, level, optname, optval, optlen)
			   : -1;
}
...
void* MongoSocket::Raw() {
	return impl_ ? impl_->sock : nullptr;
}

...
int l_txn_opts_new(lua_State* L) {
	auto* opts = CLOUDENGINE_MEM_NEW_NOTHROW(mongo::MongoTransactionOpts);
	if (!opts) {
		lua_pushnil(L);
...
int l_txn_opts_destroy(lua_State* L) {
	l_txn_opts_gc(L);
	return 0;
}
...
int l_txn_opts_clone(lua_State* L) {
	...
	auto* copy = CLOUDENGINE_MEM_NEW_NOTHROW(mongo::MongoTransactionOpts, opts->Clone());
	if (!copy) {
		lua_pushnil(L);
		lua_pushstring(L, "allocation failure");
		lua_pushnil(L);

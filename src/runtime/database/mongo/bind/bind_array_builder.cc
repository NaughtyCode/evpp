...
int l_array_builder_destroy(lua_State* L) {
	l_array_builder_gc(L);
	return 0;
}
...
int l_array_builder_append_binary(lua_State* L) {
	...
	lua_pushboolean(L,
					builder && len <= UINT32_MAX &&
						builder->AppendBinary(subtype,
...
int l_array_builder_append_timestamp(lua_State* L) {
	...
	lua_pushboolean(
		L,
		builder && ts >= 0 && ts <= UINT32_MAX && inc >= 0 && inc <= UINT32_MAX &&
...
int l_array_builder_append_document_end(lua_State* L) {
	...
	auto* subdoc = GetUserdata<mongo::BsonDocument>(L, 2, "bson.doc");
	lua_pushboolean(
		L, builder && subdoc && mongo::BsonArrayBuilder::AppendDocumentEnd(builder, subdoc));
	return 1;
}
...
int l_array_builder_append_array_builder_end(lua_State* L) {
	...
	lua_pushboolean(
		L, builder && child && mongo::BsonArrayBuilder::AppendArrayBuilderEnd(builder, child));
	return 1;
}

...
int l_coll_gc(lua_State* L) {
	...
	CLOUDENGINE_MEM_DELETE(coll);
	*CheckUserdata<mongo::MongoCollection>(L, 1, kMetaName) = nullptr;
	return 0;
}

int l_coll_destroy(lua_State* L) {
	l_coll_gc(L);
	return 0;
}
...
int l_coll_get_read_prefs(lua_State* L) {
	auto* coll = GetUserdata<mongo::MongoCollection>(L, 1, kMetaName);
	if (!coll || !coll->GetReadPrefs())
		lua_pushnil(L);
	else
		lua_pushlightuserdata(L, const_cast<void*>(coll->GetReadPrefs()));
	return 1;
}

int l_coll_get_write_concern(lua_State* L) {
	auto* coll = GetUserdata<mongo::MongoCollection>(L, 1, kMetaName);
	if (!coll || !coll->GetWriteConcern())
		lua_pushnil(L);
	else
		lua_pushlightuserdata(L, const_cast<void*>(coll->GetWriteConcern()));
	return 1;
}

int l_coll_get_read_concern(lua_State* L) {
	auto* coll = GetUserdata<mongo::MongoCollection>(L, 1, kMetaName);
	if (!coll || !coll->GetReadConcern())
		lua_pushnil(L);
	else
		lua_pushlightuserdata(L, const_cast<void*>(coll->GetReadConcern()));
	return 1;
}

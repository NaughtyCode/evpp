...
int l_write_concern_gc(lua_State* L) {
	...
	CLOUDENGINE_MEM_DELETE(concern);
	*CheckUserdata<mongo::MongoWriteConcern>(L, 1, kMetaName) = nullptr;
	return 0;
}

int l_write_concern_new(lua_State* L) {
	auto* concern = CLOUDENGINE_MEM_NEW_NOTHROW(mongo::MongoWriteConcern);
	if (!concern) {
		lua_pushnil(L);
...
int l_write_concern_copy(lua_State* L) {
	...
	auto* copy = CLOUDENGINE_MEM_NEW_NOTHROW(mongo::MongoWriteConcern, concern->Copy());
	if (!copy) {
		lua_pushnil(L);
		lua_pushstring(L, "allocation failure");
		lua_pushnil(L);

...
void MongoCursor::SetLimit(int64_t limit) {
	if (impl_ && impl_->cursor) mongoc_cursor_set_limit(impl_->cursor, limit);
}
...
void MongoCursor::SetServerId(uint32_t server_id) {
	if (impl_ && impl_->cursor) mongoc_cursor_set_server_id(impl_->cursor, server_id);
}
...
void MongoCursor::SetMaxAwaitTimeMs(uint32_t max_await_ms) {
	if (impl_ && impl_->cursor) mongoc_cursor_set_max_await_time_ms(impl_->cursor, max_await_ms);
}

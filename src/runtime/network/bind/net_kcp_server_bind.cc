...
void ReleaseKcpServer(lua_State* L, KcpServerCtx* ctx) {
	...
	if (loop && loop->IsRunning()) {
		...
		g_kcp_pending_unref.AddRef(L, old_msg_ref);
		for (int ref : retired_refs) {
			g_kcp_pending_unref.AddRef(L, ref);
		}
		g_kcp_pending_unref.AddRef(L, old_inst_ref);
		loop->QueueInLoop([L, old_msg_ref, old_inst_ref, retired_refs = std::move(retired_refs), ctx] {
			if (g_kcp_alive.TryAcquire()) {
				if (old_msg_ref != LUA_NOREF) {
					...
					g_kcp_pending_unref.RemoveRef(L, old_msg_ref);
				}
				for (int ref : retired_refs) {
					if (ref != LUA_NOREF) {
						luaL_unref(L, LUA_REGISTRYINDEX, ref);
						g_kcp_pending_unref.RemoveRef(L, ref);
					}
				}
				if (old_inst_ref != LUA_NOREF) {
					luaL_unref(L, LUA_REGISTRYINDEX, old_inst_ref);
					g_kcp_pending_unref.RemoveRef(L, old_inst_ref);
				}
				g_kcp_alive.Release();
			}

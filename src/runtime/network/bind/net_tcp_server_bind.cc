...

	{
		...
		ctx->server->SetConnectionCallback(
			...
			if (conn->IsConnected()) {
				...
				auto* logger = GetLogger();
				ENGINE_LOG_INFO(
					logger, "[net.server] new conn: conn=[{}] from=[{}]", conn->id(), remote);

				CallInstMethodTableStr(L_ptr, server_inst_ref, "on_connect", conn_inst_ref, remote);
				...
				if (!ctx_ptr->disposed) {
					...
					if (HasMethod(L_ptr, conn_ref, "on_close")) {
						...
						CallInstMethodTableStr(L_ptr, sv_ref, "on_close", conn_ref, remote);
					}
				}

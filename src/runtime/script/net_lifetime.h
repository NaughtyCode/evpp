...
// ═══════════════════════════════════════════════════════════════════════════
// NetAliveGuard — generalized cross-thread lifetime guard for RunInLoop
// callbacks that capture lua_State* or Lua registry references.
//
// Usage:
...
public:
     ...
     *   1. Fast-path: check alive_ without the mutex (common case: alive).
     ...
     * TryAcquire/Release scope (would deadlock on the same thread). */
    void WaitDrain();

...
struct DecodeCursor {
	...
	DecodeCursor(const unsigned char* s, size_t len) noexcept : p(s), left(len) {
	}
...
void EncodeBytes(EncodeBuf& buf, const unsigned char* s, size_t len) {
	if (len < 32) {
		...
		uint8_t hdr[3] = {
			0xda, static_cast<uint8_t>((len >> 8) & 0xff), static_cast<uint8_t>(len & 0xff)};
		buf.Append(hdr, 3);
	} else {
		uint8_t hdr[5] = {0xdb,
...
void EncodeDouble(EncodeBuf& buf, double d) {
	static_assert(sizeof(float) == 4 && sizeof(double) == 8, "unexpected float/double size");
...
void EncodeInt(EncodeBuf& buf, int64_t n) {
	if (n >= 0) {
		if (n <= 127) {
			...
			uint8_t b[3] = {
				0xcd, static_cast<uint8_t>((n >> 8) & 0xff), static_cast<uint8_t>(n & 0xff)};
			buf.Append(b, 3);
		...
		if (n >= -32) {
			...
			uint8_t b[3] = {
				0xd1, static_cast<uint8_t>((un >> 8) & 0xff), static_cast<uint8_t>(un & 0xff)};
			buf.Append(b, 3);
...
void EncodeArray(EncodeBuf& buf, int64_t n) {
	if (n <= 15) {
		...
		uint8_t b[3] = {
			0xdc, static_cast<uint8_t>((n >> 8) & 0xff), static_cast<uint8_t>(n & 0xff)};
		buf.Append(b, 3);
	} else {
		uint8_t b[5] = {0xdd,
						static_cast<uint8_t>((n >> 24) & 0xff),
...
void EncodeMap(EncodeBuf& buf, int64_t n) {
	if (n <= 15) {
		...
		uint8_t b[3] = {
			0xde, static_cast<uint8_t>((n >> 8) & 0xff), static_cast<uint8_t>(n & 0xff)};
		buf.Append(b, 3);
	} else {
		uint8_t b[5] = {0xdf,
						static_cast<uint8_t>((n >> 24) & 0xff),
...
void DecodeToLuaType(lua_State* L, DecodeCursor* c, int depth) {
	...
	case 0xcd:	// uint 16
		if (!c->Need(3)) return;
		lua_pushinteger(L,
						static_cast<lua_Integer>((static_cast<uint16_t>(c->p[1]) << 8) |
	...
	case 0xce:	// uint 32
		...
		PushUnsigned(L,
					 (static_cast<uint32_t>(c->p[1]) << 24) |
						 ...
						 (static_cast<uint32_t>(c->p[3]) << 8) | static_cast<uint32_t>(c->p[4]));
		c->Consume(5);
		break;
	case 0xd2:	// int 32
		if (!c->Need(5)) return;
		{
			uint32_t u32 = (static_cast<uint32_t>(c->p[1]) << 24) |
						   (static_cast<uint32_t>(c->p[2]) << 16) |
						   (static_cast<uint32_t>(c->p[3]) << 8) | static_cast<uint32_t>(c->p[4]);
			int32_t i32;
	...
	case 0xcf:	// uint 64
		...
		PushUnsigned(
			L,
			(static_cast<uint64_t>(c->p[1]) << 56) | (static_cast<uint64_t>(c->p[2]) << 48) |
				(static_cast<uint64_t>(c->p[3]) << 40) | (static_cast<uint64_t>(c->p[4]) << 32) |
				(static_cast<uint64_t>(c->p[5]) << 24) | (static_cast<uint64_t>(c->p[6]) << 16) |
				(static_cast<uint64_t>(c->p[7]) << 8) | static_cast<uint64_t>(c->p[8]));
		c->Consume(9);
		break;
	case 0xd3:	// int 64
		if (!c->Need(9)) return;
	...
	case 0xdb:	// str 32
		if (!c->Need(5)) return;
		{
			size_t l = (static_cast<size_t>(c->p[1]) << 24) | (static_cast<size_t>(c->p[2]) << 16) |
					   (static_cast<size_t>(c->p[3]) << 8) | static_cast<size_t>(c->p[4]);
...
void ExportMsgPack(ScriptVM& vm) {
	...
	ENGINE_LOG_INFO(logger,
					"ScriptBind: msgpack modules exported "

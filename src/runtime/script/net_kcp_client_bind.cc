...
int l_kcp_client_connect_static(lua_State* L) {
    ...
    if (!ctx->client->Connect(host, port, conv)) {
        ...
        ENGINE_LOG_ERROR(logger, "[net.kcp_client] failed to connect to {}:{} conv={}", host, port, conv);
        ctx->disposed = true;
        lua_pushnil(L);
        lua_setfield(L, -2, "_ctx");
        delete ctx;
...
int l_kcp_client_connect(lua_State* L) {
    ...
    if (!ctx->client->Connect(host, port, ctx->conv)) {
        auto* logger = GetLogger();
        ENGINE_LOG_ERROR(logger, "[net.kcp_client] failed to connect to {}:{} conv={}", host, port, ctx->conv);
        lua_pushboolean(L, 0);
        lua_pushfstring(L, "kcp connect failed: %s:%d", host, port);
        return 2;
    }
...
int l_kcp_client_do_request_static(lua_State* L) {
    ...
    std::string resp = evpp::kcp::sync::Client::DoRequest(host, port,
        std::string(data, len), timeout_ms, conv);
    lua_pushlstring(L, resp.data(), resp.size());
    return 1;
}

...
// On success, nresults are on the stack. On error, stack is clean.
inline LuaCallResult SafeCallLua(lua_State* L, LuaCallOptions opts) {
	...
	if (!lua_isfunction(L, func_idx)) {
		lua_pop(L, opts.nargs + 1);  // pop args + non-function
	...
	int result = lua_pcall(L, opts.nargs, opts.nresults, func_idx);

	if (result != LUA_OK) {
		if (opts.log_on_error) {
			const char* err = lua_tostring(L, -1);
			if (err && (!opts.throttle_repeated || ShouldLogError(err))) {
				// Error already includes traceback from error handler
			}
		}
		lua_pop(L, 1);  // pop error message
		lua_remove(L, func_idx);  // remove error handler

...
static constexpr float ModelRectangleMergeAngle =
    1.1f;  // treat face normal angle smaller than 1.1 as plane quad

static bool SaveSimplifyModel = false;
static std::string outputSaveDirectory = "D:\\Temp";
...
void OccluderQuad::ConfigControlParameters(bool isTerrain) {
  ...
  if (isTerrain) {
    ...
    PlaneQuadDotThreshold =
        ProjectLineMidAngleTh;  // cos(5 degree)  //treat face normal angle
                                // smaller than 5 as plane quad
    QuadFourSameFaceTh =
        ProjectLineMidAngleTh;  // cos(5 degree)  //treat face normal angle
                                // smaller than 5 as plane quad
    QuadTwoSameFaceTh = ProjectLineMidAngleTh;  // cos(3 degree)    //do not
                                                // relax this constraints
  } else {
    // very strict controlling parameters
    RightAngleUnitDotTh = cos(ModelRectangleAngle * degreeToRadian);
    // cos(89f * 3.1415926f / 180);
    ProjectLineMidAngleTh = cos(3 * degreeToRadian);  // cos(3 degree)
    PlaneQuadDotThreshold = QuadFourSameFaceTh = QuadTwoSameFaceTh =
        cos(ModelRectangleMergeAngle * degreeToRadian);
  }
}

static constexpr float QuadFourSameSquareAreaTh =
    0.9f;  // only merge roughly same area with threshold sqrt(0.9) =
           // 0.94868329805

enum TerrainOpMode {
  kTerrainOpNone,
  kTerrainOpSafeMerging,
  kTerrainOpAggressiveMerging,
};

static int gTerrainGridOptimization =
    TerrainOpMode::kTerrainOpAggressiveMerging;  // this bool would be able to
...
static bool StoreModel(const std::string& file_path, const float* vertices,
                       int nVert, const uint16_t* indices, int indicesNum) {
...
static __m128 Normalize(__m128 v) {
  __m128 lengthSquare = _mm_dp_ps(v, v, 0x7F);
  __m128 nonZero = _mm_cmpgt_ps(lengthSquare, _mm_setzero_ps());
  return _mm_and_ps(_mm_mul_ps(v, _mm_rsqrt_ps(lengthSquare)), nonZero);
}

static float DotSum3(__m128 a, __m128 b) {
  __m128 d = _mm_mul_ps(a, b);
  alignas(16) float values[4] = {};
  _mm_store_ps(values, d);
...
void OccluderQuad::CheckRectangleQuad(MeshQuad* q, __m128* vertices) {
  ...
  q->IsRectangle = dot0 < RightAngleUnitDotTh && dot1 < RightAngleUnitDotTh &&
                   dot2 < RightAngleUnitDotTh && dot3 < RightAngleUnitDotTh;
}

void OccluderQuad::HandleSquareTerrainInput(
    OccluderBakeBuffer* pBakeBuffer, const uint16_t* indices, unsigned int nIdx,
    const float* inputVertices, int nVert, QuadTriIndex& qti, float quadAngle,
    int squareTerrainXPoints) {
  ...
  if (xUnit == 0 || yUnit == 0) {
    OccluderQuad::DecomposeToQuad(pBakeBuffer, indices, nIdx, inputVertices,
                                  nVert, qti, quadAngle, nullptr, true);
    return;
  }
  ...
  std::vector<uint16_t> gridMap(gridPointNum, 0);
  ...
  if (IsTerrain == false) {
    IsTerrain =
        activeCount == maxGridIdx && (gridWidth * gridWidth == totalGrids);
  ...
  if (IsTerrain) {
    ...
    for (unsigned int idx = 0; idx < nIdx; idx += 3) {
      ...
      int gridIdx0 =
          TerrainGridY[vertIdx0] * axisPoints + TerrainGridX[vertIdx0];

      int vertIdx1 = indices[idx + 1];
      int gridIdx1 =
          TerrainGridY[vertIdx1] * axisPoints + TerrainGridX[vertIdx1];

      int vertIdx2 = indices[idx + 2];
      int gridIdx2 =
          TerrainGridY[vertIdx2] * axisPoints + TerrainGridX[vertIdx2];

      if (gridIdx0 != gridIdx1 && gridIdx1 != gridIdx2 &&
          gridIdx0 != gridIdx2) {
    ...
    ti.gGridX = TerrainGridX.data();
    ti.gGridY = TerrainGridY.data();
    ti.gGridToVertID = GridToVert.data();
    //*********************************************************************

    //	storeModel("D:\\Temp\\" + std::to_string(DebugOccluderIdx++) +
    //"Input.off", inputVertices, nVert, &vertexIndices[0],
    // vertexIndices.size());
    ...
    if (vertexIndices.empty()) {
      ...
      OccluderQuad::DecomposeToQuad(pBakeBuffer, vertexIndices.data(),
                                    static_cast<uint32_t>(vertexIndices.size()),
                                    inputVertices, nVert, qti, quadAngle, &ti,
                                    true);
    }
  } else {
    OccluderQuad::DecomposeToQuad(pBakeBuffer, indices, nIdx, inputVertices,
...
#ifdef CULLING_ENGINE_SUPPORT_ALL_FEATURES
static void LoadObj(const char* path, float quadAngle,
                    int squareTerrainXPoints) {
  std::vector<uint16_t> vertexIndices;
  ...
  OccluderBakeBuffer pbuffer;
  if (!vertexIndices.empty() && !temp_vertices.empty()) {
    if (OccluderQuad::FitTerrainSquareProperty(nIdx, nVert,
                                               squareTerrainXPoints) ||
        squareTerrainXPoints > 0) {
      OccluderQuad::HandleSquareTerrainInput(
          &pbuffer, vertexIndices.data(), nIdx, temp_vertices.data(), nVert,
          qti, quadAngle, squareTerrainXPoints);
    } else {
      OccluderQuad::DecomposeToQuad(&pbuffer, vertexIndices.data(), nIdx,
                                    temp_vertices.data(), nVert, qti,
                                    quadAngle);
    }
  }
  fclose(file);
}
static void LoadOff(const char* path, float quadAngle,
                    int squareTerrainXPoints) {
  std::vector<uint16_t> vertexIndices;
  std::vector<float> temp_vertices;
  ...
  while (1) {
    ...
    if (strcmp(lineHeader, "OFF") == 0) {
      ...
      if (!vertexIndices.empty() && !temp_vertices.empty()) {
        QuadTriIndex qti;
        OccluderBakeBuffer pbuffer;
        OccluderQuad::DecomposeToQuad(&pbuffer, vertexIndices.data(),
                                      static_cast<unsigned int>(nFaces) * 3,
                                      temp_vertices.data(), nVerts, qti,
                                      quadAngle, nullptr);
      }
    }
  }
...
bool OccluderQuad::FitTerrainSquareProperty(uint32_t nIdx, uint32_t nVert,
  ...
  if (nVert >= (uint32_t)gDelayTerrainOccluderVertTh) {
    ...
    if (gridWidth * gridWidth == totalGrids) {
      ...
      if (nVert == maxGridPoint || nVert == maxGridPoint + gridWidth * 2 + 3) {
        return true;
      }
    }
  }
...
CullingEngine::OccluderQuad::MeshFace* OccluderBakeBuffer::GetFaceBySize(
    uint32_t size) {
  if (mFaces.size() < size) {
...
CullingEngine::OccluderQuad::MeshQuad* OccluderBakeBuffer::GetQuadBySize(
    uint32_t size) {
  if (mQuads.size() < size) {
...
CullingEngine::OccluderQuad::MeshVertex* OccluderBakeBuffer::GetVertexBySize(
    uint32_t size) {
...
const uint16_t* OccluderBakeBuffer::ReIndex(const uint16_t* indices,
                                            const float* inputVertices,
                                            int vertNum, unsigned int nIdx) {
  // assert(nIdx == vertNum);
...
void OccluderQuad::DecomposeToQuad(OccluderBakeBuffer* pBakeBuffer,
                                   const uint16_t* indices, unsigned int nIdx,
                                   const float* inputVertices, int vertNum,
                                   QuadTriIndex& qti, float quadAngle,
                                   TerrainInput* terrain, bool checkedTerrain,
                                   int terrainGridAxisPoint) {
  bool isTerrain = terrain != nullptr;

  if (gTerrainGridOptimization != TerrainOpMode::kTerrainOpNone &&
      checkedTerrain == false) {
    if (FitTerrainSquareProperty(nIdx, vertNum, terrainGridAxisPoint) ||
        terrainGridAxisPoint > 0) {
      HandleSquareTerrainInput(pBakeBuffer, indices, nIdx, inputVertices,
                               vertNum, qti, quadAngle, terrainGridAxisPoint);
      return;
    }
  }
  ...
  float cosQuadAngle =
      cos(15.0f * 3.1415926f /
          180);  // default value, expect to be evaluated by compiler
  if (quadAngle != 15) {
  ...
  if (nTri > 65535 || nIdx <= 12 ||
      quadAngle == 0)  // not possible to form a Quad
  {
    qti.indices = nullptr;
    qti.nQuads = 0;
    qti.nTriangles = nIdx;
  ...
  for (uint32_t i_tri = 0; i_tri < nTri; ++i_tri) {
    ...
    f->UnitNormal = Normalize(e1e3Cross);
    ...
    // cross approach
    // float area2 = 0.5 * sqrt(dotSum3(e1e2Cross, e1e2Cross));
    f->SquareArea = DotSum3(e1e3Cross, e1e3Cross);
...

    {
      ...
      for (auto e = neighbors; e != nullptr; e = e->Next) {
        if (e->FaceId >= f->FaceID) {
          ...
          // we only test each face with the smaller face to avoid duplicate
          // calculation up to this stage, all the face would be larger than
          // faceID, so use break, not continue
        ...
        if (e->endVert == fv) {
          ...
          if (eface->PairFace == nullptr) {
            ...
            // allow CullingEngine Quad drawing
            if (dotSum >= cosQuadAngle) {
              ...
              f->IsPlanarQuad = eface->IsPlanarQuad =
                  (dotSum >= PlaneQuadDotThreshold) && bSimplifyMesh;
            }
          }
          break;
        }
  ...
  if (SaveSimplifyModel) {
    std::string file_path = outputSaveDirectory + "\\" +
                            std::to_string(DebugOccluderIdx++) + "input.off";
  ...
  if (bSimplifyMesh) {
    ...
    // Optimization: check whether it is a simple quad and output the quad if
    // conditions allow
    if (xFlat + yFlat + zFlat == 1) {
      ...
      if (allSameDirection) {
        float area = 0;
        if (xFlat)
          area = aabbf[1] * aabbf[2];
        else if (yFlat)
          area = aabbf[0] * aabbf[2];
        else
          area = aabbf[0] * aabbf[1];
        float areaSum = 0;
        for (uint32_t i_tri = 0; i_tri < nTri; ++i_tri) {
          MeshFace* f = &Faces[i_tri];
          areaSum += sqrt(f->SquareArea);
        }
        areaSum *= 0.5f;
        float th = 0.0001f;
        ...
        if (areaSum > areaThMin && areaSum < areaThMax) {
          ...
          if (SaveSimplifyModel) {
            std::string file_path = outputSaveDirectory + "\\" +
                                    std::to_string(DebugOccluderIdx++) +
                                    "outQuad.off";
            ...
            if (fileWriter != nullptr) {
              ...
              for (uint32_t i_vert = 0; i_vert < 4; ++i_vert) {
                fprintf(fileWriter, "%f %f %f\n", vertf[i_vert * 4],
                        vertf[(i_vert * 4) + 1], vertf[(i_vert * 4) + 2]);
              }
  ...
  std::fill_n(Quads, totalRequestQuads, MeshQuad{});
  uint32_t quadIdx = 0;
  ...
  if (toSimplifyMesh) {
    ...
    if (quadMerge) {
      ...
      for (int round = 0; round < mergeRound; round++) {
        ...
        for (uint32_t idx = 0; idx < nVert; idx++) {
          ...
          if (isTerrain) {
            bool okX =
                (terrain->gGridX[idx] & terrainCheckMask) == terrainMergeIdx;
            bool okY =
                (terrain->gGridY[idx] & terrainCheckMask) == terrainMergeIdx;
            ...
            if (v->RectangleDegree == 2) {
              ...
              if (MergeQuad(q0, q1, points, Vertices, isTerrain,
                            quadFourMergeCount))
                continue;
              ...
              if (MergeQuad(q0, q1, points, Vertices, isTerrain,
                            quadFourMergeCount))
                continue;
              if (MergeQuad(q0, q2, points, Vertices, isTerrain,
                            quadFourMergeCount))
                continue;
              if (MergeQuad(q1, q2, points, Vertices, isTerrain,
                            quadFourMergeCount))
                continue;
            }
          }
          if (v->RectangleDegree == 4)  // pivot point
          {
            ...
            if (q0->QuadKids != targetQuadIdx ||
                q1->QuadKids != targetQuadIdx ||
                q2->QuadKids != targetQuadIdx ||
                q3->QuadKids != targetQuadIdx) {
              continue;
            }

            float largestArea =
                std::max<float>(q0->face1->SquareArea, q1->face1->SquareArea);
            largestArea = std::max<float>(largestArea, q2->face1->SquareArea);
            largestArea = std::max<float>(largestArea, q3->face1->SquareArea);
            float areaTh = largestArea * QuadFourSameSquareAreaTh;
            if (q0->face1->SquareArea > areaTh &&
                q1->face1->SquareArea > areaTh &&
                q2->face1->SquareArea > areaTh &&
                q3->face1->SquareArea > areaTh) {
              ...
              if (t1 >= QuadFourSameFaceTh && t2 >= QuadFourSameFaceTh &&
                  t3 >= QuadFourSameFaceTh) {
                if (q0->MergeWith(q1, points, Vertices, isTerrain,
                                  quadFourMergeCount)) {
                  if (q2->MergeWith(q3, points, Vertices, isTerrain,
                                    quadFourMergeCount))
                    q0->MergeWith(q2, points, Vertices, isTerrain,
                                  quadFourMergeCount);
                } else if (q0->MergeWith(q2, points, Vertices, isTerrain,
                                         quadFourMergeCount)) {
                  if (q1->MergeWith(q3, points, Vertices, isTerrain,
                                    quadFourMergeCount))
                    q0->MergeWith(q1, points, Vertices, isTerrain,
                                  quadFourMergeCount);
                } else if (q0->MergeWith(q3, points, Vertices, isTerrain,
                                         quadFourMergeCount)) {
                  if (q1->MergeWith(q2, points, Vertices, isTerrain,
                                    quadFourMergeCount))
                    q0->MergeWith(q1, points, Vertices, isTerrain,
                                  quadFourMergeCount);
                } else if (q2->MergeWith(q3, points, Vertices, isTerrain,
                                         quadFourMergeCount)) {
                } else if (q2->MergeWith(q1, points, Vertices, isTerrain,
                                         quadFourMergeCount)) {
                }
              }
            }
        ...
        while (degree2Merge > 0) {
          ...
          for (uint32_t idx = 0; idx < nVert; idx++) {
            ...
            if (v->RectangleDegree == 2) {
              ...
              MeshQuad* q1 = &Quads[p1];
              if (q0->QuadKids ==
                  q1->QuadKids)  // only allow double expand to avoid extend T
              {
                float t1 =
                    DotSum3(q0->face1->UnitNormal, q1->face1->UnitNormal);
                if (t1 >= QuadTwoSameFaceTh) {
                  // TerrainAggressiveMerging set to false by default
                  // with aggressive on, the quad number would be reduced by
                  // another 10 % ~20 % however, this would require extra
                  // handling during point correction such as edge-joining to
                  // proper remove of T-junction
                  if (gTerrainGridOptimization ==
                      TerrainOpMode::kTerrainOpAggressiveMerging) {
                    q0->MergeWith(q1, points, Vertices, isTerrain,
                                  degree2Merge);
                  } else {
                    for (int i = 0; i < 4; i++) {
    ...
    if (neighborMerge && isTerrain == false && toSimplifyMesh) {
      ...
      do {
        ...
        if (planarFreeShapeMerge) {
          ProjectLineMidAngleTh =
              PlaneQuadDotThreshold;  // apply strict constraint
          for (uint32_t idx = 0; idx < nVert; idx++) {
            MeshVertex* v = &Vertices[idx];
            if (v->RectangleDegree == 4)  // pivot point
            {
              ...
              if (q0->MergeWith(q1, points, Vertices, false,
                                neighborMergeCount)) {
                q2->MergeWith(q3, points, Vertices, false, neighborMergeCount);
              } else if (q0->MergeWith(q2, points, Vertices, false,
                                       neighborMergeCount)) {
                q1->MergeWith(q3, points, Vertices, false, neighborMergeCount);
              } else if (q0->MergeWith(q3, points, Vertices, false,
                                       neighborMergeCount)) {
                q1->MergeWith(q3, points, Vertices, false, neighborMergeCount);
              }
            }
          ...
          for (uint32_t idx = 0; idx < nVert; idx++) {
            ...
            if (v->RectangleDegree == 2)  // pivot point
            {
              ...
              if (q0->MergeWith(q1, points, Vertices, false,
                                neighborMergeCount)) {
              } else if (q1->MergeWith(q2, points, Vertices, false,
                                       neighborMergeCount)) {
              } else if (q0->MergeWith(q2, points, Vertices, false,
                                       neighborMergeCount)) {
              }
          ...
          for (uint32_t idx = 0; idx < nVert; idx++) {
            ...
            if (v->RectangleDegree == 2)  // pivot point
            {
              ...
              if (qti.IsPlanar == false) {
                float largestArea = std::max<float>(q0->face1->SquareArea,
                                                    q1->face1->SquareArea);
                float areaTh = largestArea *
                               0.5f;  // allow area ratio around sqrt(0.5) = 0.7

                if (((q1->QuadKids == q0->QuadKids && q0->QuadKids > 0)) &&
                    q0->face1->SquareArea > areaTh &&
                    q1->face1->SquareArea > areaTh) {
                  float t1 =
                      DotSum3(q0->face1->UnitNormal, q1->face1->UnitNormal);
              ...
              if (t01 > QuadTwoSameFaceTh && q0->QuadKids == q1->QuadKids &&
                  q0->MergeWith(q1, points, Vertices, false,
                                neighborMergeCount)) {
                float t23 =
                    DotSum3(q2->face1->UnitNormal, q3->face1->UnitNormal);
                if (t23 > QuadTwoSameFaceTh && q2->QuadKids == q3->QuadKids) {
                  q2->MergeWith(q3, points, Vertices, false,
                                neighborMergeCount);
                }
              } else {
                float t02 =
                    DotSum3(q0->face1->UnitNormal, q2->face1->UnitNormal);
                if (t02 > QuadTwoSameFaceTh && q0->QuadKids == q2->QuadKids &&
                    q0->MergeWith(q2, points, Vertices, false,
                                  neighborMergeCount)) {
                  float t13 =
                      DotSum3(q1->face1->UnitNormal, q3->face1->UnitNormal);
                  if (t13 > QuadTwoSameFaceTh && q1->QuadKids == q3->QuadKids) {
                    q1->MergeWith(q3, points, Vertices, false,
                                  neighborMergeCount);
                  }
                } else {
                  float t03 =
                      DotSum3(q0->face1->UnitNormal, q3->face1->UnitNormal);
                  if (t03 > QuadTwoSameFaceTh && q0->QuadKids == q3->QuadKids &&
                      q0->MergeWith(q3, points, Vertices, false,
                                    neighborMergeCount)) {
                    float t12 =
                        DotSum3(q1->face1->UnitNormal, q2->face1->UnitNormal);
                    if (t12 > QuadTwoSameFaceTh &&
                        q1->QuadKids == q2->QuadKids) {
                      q1->MergeWith(q2, points, Vertices, false,
                                    neighborMergeCount);
                    }
                  }
                }
        ...
        if (neighborMergeCount < terminateTh && planarFreeShapeMerge == false &&
            qti.IsPlanar) {
    ...
    if (isTerrain) {
      ...
      if (mergedQuadNum > 0) {
        ...
        std::vector<RectEdge> EdgePool(totalLargeQuad * 4);
        int EdgePoolIdx = 0;
            ...
            for (int idx = 0; idx < 4 && validTerrain; idx++) {
              ...
              if (validTerrain) {
                ...
                if (length > 1) {
                  ...
                  int StartGrid = re->startGridX +
                                  re->startGridY * terrain->terrainAxisSize;
                  re->Key = StartGrid;
                  re->Key <<= 16;
                  re->Key |= 65535 - length;
                  if (startGridX == endGridX) {
                    re->Key = (((uint64_t)startGridX << 16) + re->startGridY)
                              << 32;
                    re->Key += 65535 - length;
                  }
        ...
        // validTerrain = false;
        if (validTerrain && !mergedEdges.empty()) {
          // time to seal holes. With square terrain assumption, we can derive
          // all points on edge sort in the order of
          // edge-length/quadkids/smallGridIdx/endGridIdx Warning: might not be
          // able to converge if aggressiveness on
          std::sort(mergedEdges.begin(), mergedEdges.end(),
                    RectEdge::AscendingSort);
          ...
          for (int idx = 1; idx < mergedEdges.size(); idx++) {
            ...
            // std::cout << "Edge " << edge->startGridX << " " << edge->endGridX
            // << " " << edge->startGridY << " " << edge->endGridY << std::endl;
          ...
          for (auto re : mergedEdges) {
            ...
            int StartGrid =
                re->startGridX + re->startGridY * terrain->terrainAxisSize;
            int endGrid =
                re->endGridX + re->endGridY * terrain->terrainAxisSize;
            int length = (endGridX - startGridX) + (endGridY - startGridY);
          ...
          std::sort(mergedEdges.begin(), mergedEdges.end(),
                    RectEdge::DescendingSort);
          // Two round to eliminate T junctions
          int round = 2;
          round +=
              gTerrainGridOptimization ==
              TerrainOpMode::kTerrainOpAggressiveMerging;  // one more round for
                                                           // aggressive mode
          for (int updateIdx = 0; updateIdx < round; updateIdx++) {
            for (int idx = 0; idx < validCount; idx++) {
              ...
              uint32_t startVert =
                  terrain->gGridToVertID[startGridX +
                                         startGridY * terrain->terrainAxisSize];
              uint32_t endVert =
                  terrain->gGridToVertID[endGridX +
                                         endGridY * terrain->terrainAxisSize];
        ...
        if (validTerrain == false) {
          DecomposeToQuad(pBakeBuffer, indices, nIdx, inputVertices, vertNum,
                          qti, quadAngle, nullptr, checkedTerrain, -1);
        }
      }
    }
  } else {
...

  {
    if (SaveSimplifyModel) {
      inputVertices =
          (float*)qti.verts;  // apply water tight fix to avoid any small holes

      std::string file_path;
      if (qti.nQuads > 0) {
        if (isTerrain) {
          file_path = outputSaveDirectory + "\\" +
                      std::to_string(DebugOccluderIdx++) + "TerrainQuad.off";
        } else {
          file_path = outputSaveDirectory + "\\" +
                      std::to_string(DebugOccluderIdx++) + "QuadOut.off";
        }
        ...
        if (fileWriter != nullptr) {
          ...
          for (uint32_t i = 0; i < qti.nQuads; i += 4) {
            fprintf(fileWriter, "4 %d %d %d %d\n", qti.indices[i],
                    qti.indices[i + 1], qti.indices[i + 2], qti.indices[i + 3]);
          }
          for (uint32_t i = qti.nQuads; i < totalUseIdx; i += 3) {
            fprintf(fileWriter, "3 %d %d %d\n", qti.indices[i],
                    qti.indices[i + 1], qti.indices[i + 2]);
          }
      ...
      // according to the triangle off file to store the simplified mesh if wish
      // to push triangle mesh to CullingEngine save as triangle off file
      file_path = outputSaveDirectory + "\\" +
                  std::to_string(DebugOccluderIdx++) + "out.off";
      FILE* fileWriter = fopen(file_path.c_str(), "w");
      if (fileWriter != nullptr) {
        ...
        for (uint32_t i = 0; i < qti.nQuads; i += 4) {
          // save quads to two triangles
          fprintf(fileWriter, "3 %d %d %d\n", qti.indices[i],
                  qti.indices[i + 2], qti.indices[i + 1]);

          fprintf(fileWriter, "3 %d %d %d\n", qti.indices[i],
                  qti.indices[i + 3], qti.indices[i + 2]);
        }
        for (uint32_t i = qti.nQuads; i < totalUseIdx; i += 3) {
          fprintf(fileWriter, "3 %d %d %d\n", qti.indices[i],
                  qti.indices[i + 1], qti.indices[i + 2]);
...
bool OccluderQuad::MergeQuad(MeshQuad* q0, MeshQuad* q1, __m128* points,
                             MeshVertex* Vertices, bool isTerrain,
                             int& totalMerged) {
  if (q1->QuadKids == 0 || q0->QuadKids == 0) {
    return false;
  }
  ...
  float largestArea =
      std::max<float>(q0->face1->SquareArea, q1->face1->SquareArea);
  float areaTh = largestArea * 0.5f;  // allow area ratio around sqrt(0.5) = 0.7
  if (q1->QuadKids == q0->QuadKids && ((q0->IsRectangle && q1->IsRectangle)) &&
      q0->face1->SquareArea > areaTh && q1->face1->SquareArea > areaTh) {
    float t1 = DotSum3(q0->face1->UnitNormal, q1->face1->UnitNormal);
...
bool OccluderQuad::MeshQuad::MergeWith(MeshQuad* q, __m128* vertices,
                                       MeshVertex* MeshVertices, bool isTerrain,
                                       int& TotalMerged) {
  ...
  if (find == 4) {
    ...
    if (v0First != v20First) {
      ...
      if (isTerrain == false) {
        ...
        if (ProjectToLine(vertices[idx0A], vertices[idx0B], vertices[idx0],
                          updatedPoints[0]) &&
            ProjectToLine(vertices[idx2A], vertices[idx2B], vertices[idx2],
                          updatedPoints[1])) {
    ...
    if (refEdge->startGridX == refEdge->endGridX) {
      if (this->startGridX == refEdge->startGridX) {
        if (refEdge->endGridY >= this->endGridY &&
            refEdge->startGridY <= this->startGridY) {
          return true;  // do nothing, refEdge contain current Edge
        } else if (refEdge->endGridY > this->startGridY) {
          if (refEdge->endGridY < this->endGridY) {
            // std::cout << "endY from " << refEdge->endGridY << " to " <<
            // this->endGridY << std::endl;
    ...
    if (refEdge->startGridY == refEdge->endGridY) {
      if (this->startGridY == refEdge->startGridY) {
        if (refEdge->endGridX >= this->endGridX &&
            refEdge->startGridX <= this->startGridX) {
          return true;  // do nothing, refEdge contain current Edge
        } else if (refEdge->endGridX > this->startGridX) {
          if (refEdge->endGridX < this->endGridX) {
            // std::cout << "endX from " << refEdge->endGridX << " to " <<
            // this->endGridX << std::endl;
...
// CullingEngine_SetTerrainRectangleAngle  [80, 89]
void OccluderQuad::SetTerrainRectangleAngle(unsigned int configValue) {
  TerrainRectangleAngle = (float)configValue;
}
// CullingEngine_SetTerrainMergeAngle  [1, 10]
void OccluderQuad::SetTerrainRectangleMergeAngle(unsigned int configValue) {
  TerrainRectangleMergeAngle = (float)configValue;
}

void OccluderQuad::SetSaveModel(bool v) { SaveSimplifyModel = v; }

// CullingEngine_SimplifyMeshDuringBake
void OccluderQuad::AllowPlanarQuadMerge(bool allow) { bSimplifyMesh = allow; }

// CullingEngine_TerrainGridControlMergeDuringBake
void OccluderQuad::SetTerrainGridOptimization(int value) {
  gTerrainGridOptimization = value;
}

void OccluderQuad::SetRectangleDegreeToZero(MeshVertex* Vertices,
                                            uint32_t nVert) {
...
// due to usage of singleton, this function is not thread-safe
unsigned short* OccluderQuad::CullingEngineMeshBake(
    CullingEngine::OccluderBakeBuffer* pBakeBuffer, int* outputCompressSize,
    ...
    bool counterClockWise, int TerrainGridAxisPoint) {
  {
    int faceNum = nIdx / 3;
    int batchNum = (faceNum + 3) >> 2;
    if (batchNum > 1048575) {
      *outputCompressSize = -1;
      // CULLING_ENGINE_LOG_WARNING("Input model is too large to compress for
      // CullingEngine");
      return nullptr;
    }
  }
  ...
  if (nVert <= 0 || nIdx < 3 || nIdx % 3 != 0) {
    *outputCompressSize = -2;
    return nullptr;  // invalid input;
  }

  // now verify whether the model is a valid model
  for (uint32_t idx = 0; idx < nIdx; idx++) {
    if (indices[idx] >= nVert) {
      *outputCompressSize = -3;
      return nullptr;  // this is invalid model input
    }
  }

  CullingEngine::QuadTriIndex quadData;
  CullingEngine::OccluderQuad::DecomposeToQuad(
      pBakeBuffer, indices, nIdx, vertices, nVert, quadData, quadAngle, nullptr,
      false, TerrainGridAxisPoint);
  ...
  int compressSize =
      tillQuadFinish + (triangleBatchNum * 5) - (triangleBatchNum >> 1);
  if (CullingEngine::bEnableCompressMode) {
    compressSize = quadBatchNum * 32 + triangleBatchNum * 24 +
                   quadData.nActiveVerts * 3 * 2 + 16 * 2;  // +1000;
  }

  int faceBufferSize = (int)pBakeBuffer->mFaces.size() *
                       sizeof(CullingEngine::OccluderQuad::MeshFace);
  if (faceBufferSize < compressSize * 4) {
    int faceNum =
        compressSize * 4 / sizeof(CullingEngine::OccluderQuad::MeshFace) + 1;
    pBakeBuffer->mFaces.resize(faceNum);
  }
  int* outputBuffer = (int*)(&pBakeBuffer->mFaces[0]);

  if (CullingEngine::bEnableCompressMode == false) {
    memset(outputBuffer, 0,
           tillQuadFinish *
               sizeof(__m128));  // reset all to zero,  Quad request input to be
                                 // set to 0, triangle no need
  }
  // std::cout << " tillQuadFinish " << tillQuadFinish << " compressSize " <<
  // compressSize << std::endl;
  ...
  __m128 minusRefMinInvExtents = _mm_fmadd_ps(
      _mm_xor_ps(minusZero, invExtents), quadData.refMin, _mm_set1_ps(0.5f));

  __m128i* compressData = (__m128i*)outputBuffer;

  float* f = (float*)outputBuffer;
  f += 2;  // first 64 bit used to store model description
  alignas(16) float refMinf[4] = {};
  alignas(16) float extentsf[4] = {};
  _mm_store_ps(refMinf, quadData.refMin);
  _mm_store_ps(extentsf, extents);
  f[0] = refMinf[0];
  f[1] = refMinf[1];
  f[2] = refMinf[2];
  f[3] = extentsf[0];
  ...
  if (CullingEngine::bEnableCompressMode &&
    ...
    if (quadData.nActiveVerts <= CullingEngine::SuperCompressVertNum) {
      ...
      for (uint32_t idx = 0; idx < quadData.nActiveVerts; idx++) {
        __m128 v0 = _mm_fmadd_ps(quadData.verts[idx], invExtents,
      ...
      *outputCompressSize = (piIdx >> 1) + quadData.nActiveVerts * 3 + 16;
      //	std::cout << "Byte8 Super Compressize Size " <<
      //*outputCompressSize * 2 << " Pidx " << piIdx << " Origin " << nVert * 12
      //+ nIdx * 2 << " Now Point " << quadData.nActiveVerts << " OriginPoint "
      //<< nVert << std::endl;
      return (unsigned short*)
          outputBuffer;  // this is a temp buffer, the data inside should be
      ...
      *outputCompressSize = piIdx + quadData.nActiveVerts * 3 + 16;
      //	std::cout << "16 Compressize Size " << *outputCompressSize * 2
      //<< " Pidx " << piIdx << " Origin " << nVert * 12 + nIdx * 2 << " Now
      // Point " << quadData.nActiveVerts << " OriginPoint " << nVert <<
      // std::endl;
      return (unsigned short*)
          outputBuffer;  // this is a temp buffer, the data inside should be
                         // retrieved immediately.
  ...
  if (quadBatchNum > 0) {
    __m128 tempV[16];
    for (int batchIdx = 0; batchIdx < quadBatchNum;
         batchIdx++, pIndexCurrent += 16, pVertices += 6) {
      ...
      for (uint32_t j = 0; j < 4; ++j) {
        ...
        __m128 v0 =
            _mm_fmadd_ps(tempV[j + 0], invExtents, minusRefMinInvExtents);
        __m128 v1 =
            _mm_fmadd_ps(tempV[j + 4], invExtents, minusRefMinInvExtents);
        __m128 v2 =
            _mm_fmadd_ps(tempV[j + 8], invExtents, minusRefMinInvExtents);
        __m128 v3 =
            _mm_fmadd_ps(tempV[j + 12], invExtents, minusRefMinInvExtents);
        ...
        int pIdx = (j >> 1);  // 0 or 1
        pVertices[pIdx] =
            _mm_or_si128(pVertices[pIdx], X);  // 01 for X,  23 for Y, 45 for Z
  ...
  if (triangleBatchNum > 0) {
    ...
    for (int i = 0; i < triangleBatchNum; i++)  // per 4 triangle one batch
    {
      ...
      uint32_t qIdxEnd =
          std::min<uint32_t>(4, triangleFaceNum - startFaceIdx) * 3;
      ...
      for (uint32_t j = 0; j < 3; ++j) {
        ...
        __m128 v0 =
            _mm_fmadd_ps(tempV[j + 0], invExtents, minusRefMinInvExtents);
        __m128 v1 =
            _mm_fmadd_ps(tempV[j + 3], invExtents, minusRefMinInvExtents);
        __m128 v2 =
            _mm_fmadd_ps(tempV[j + 6], invExtents, minusRefMinInvExtents);
        __m128 v3 =
            _mm_fmadd_ps(tempV[j + 9], invExtents, minusRefMinInvExtents);
  ...
  *outputCompressSize = (int)used * 8;
  ...
  // }
  // std::cout << "***********ByteUsed " << (used * 16) << " Original " <<
  // (nVert * 12 + nIdx * 2) << " Ratio " << ((used * 16)  * 100.0 / (nVert * 12
  // + nIdx * 2)) << "%" << std::endl;

  return (
      unsigned short*)outputBuffer;  // this is a temp buffer, the data inside

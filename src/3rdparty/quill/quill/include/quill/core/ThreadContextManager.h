...
#if defined(_WIN32) && defined(_MSC_VER) && !defined(__GNUC__)
inline thread_local ScopedThreadContext* g_scoped_thread_context = nullptr;
#endif
...
{
#if defined(_WIN32) && defined(_MSC_VER) && !defined(__GNUC__)
  if (QUILL_UNLIKELY(!g_scoped_thread_context))
  {
    g_scoped_thread_context = new ScopedThreadContext{
      queue_type, initial_queue_capacity, unbounded_queue_max_capacity, huge_pages_policy};
  }
  return g_scoped_thread_context->get_thread_context();
#else

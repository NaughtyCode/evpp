...
#include "engine/physics/physics_diff.h"
...
std::optional<DiffPacket> GenerateDiff(
    ...
    if (lv_delta.LengthSq() > thresholds.linear_velocity_epsilon * thresholds.linear_velocity_epsilon) {
        packet.change_mask |= (1 << 2);
    }

    // Bit 3: angular velocity check
    JPH::Vec3 av_delta = current.angular_velocity - previous.angular_velocity;
    if (av_delta.LengthSq() > thresholds.angular_velocity_epsilon * thresholds.angular_velocity_epsilon) {

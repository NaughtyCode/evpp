...
BPLayerInterfaceImpl::BPLayerInterfaceImpl(const LayerConfig& config) {
    ...
    for (const auto& [obj_name, bp_name] : config.layer_mapping) {
        ...
        if (obj_it == config.object_layers.end() ||
            bp_it == config.broad_phase_layers.end()) {
...
ObjectVSBLayerFilterImpl::ObjectVSBLayerFilterImpl(
    ...
    for (const auto& [obj_name, obj_val] : config.object_layers) {
        ...
        for (const auto& [other_name, other_val] : config.object_layers) {
            JPH::BroadPhaseLayer other_bp = bp_iface.GetBroadPhaseLayer(
                JPH::ObjectLayer(other_val));
...
bool ObjectVSBLayerFilterImpl::ShouldCollide(

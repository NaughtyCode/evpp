...
TimerId TimerManager::create_simple_timer(std::function<void()> callback,
                                           ClockId clock_id) {
...
void TimerManager::start_timer_range(TimerId id, TimePoint expiry,
...
TimerId TimerManager::create_wheel_timer(TimerWheelNode::Callback callback,
                                          uint32_t flags) {
    auto entry = std::make_unique<TimerEntry>();
...
TimerId TimerManager::create_repeating_timer(Duration interval,
                                              ...
                                              ClockId clock_id) {
    auto hr_cb = [callback = std::move(callback), interval](HrTimerNode* timer) mutable -> TimerResult {
    ...
    return create_timer(hr_cb, clock_id,
...
TimerId TimerManager::create_repeating_simple_timer(Duration interval,
    ...
    return create_timer(std::move(hr_cb), clock_id,
                        TimerMode::kAbsolute | TimerMode::kRepeating);
}
...
TimerManager::ManagerStats TimerManager::stats() const {
    ...
    // Count total entries
    {
        ...
        for (const auto& [id, entry] : entries_) {
            switch (entry->kind) {
                case TimerEntry::Kind::kHrTimer:   s.total_hrtimers++; break;
                case TimerEntry::Kind::kWheelTimer: s.total_wheel_timers++; break;
                case TimerEntry::Kind::kAlarm:      s.total_alarms++; break;
            }
        }
    }
    return s;
...
void TimerManager::dump_state() const {
    ...
    std::cout << "  HRTimers (active):   " << s.active_hrtimers
              << " / " << s.total_hrtimers << "\n";
    std::cout << "  Wheel timers:        " << s.active_wheel_timers
              << " / " << s.total_wheel_timers << "\n";
    std::cout << "  Alarms (active):     " << s.active_alarms
              << " / " << s.total_alarms << "\n";
    ...
    std::cout << "  Next HR expiry:      " << hrtimer_mgr_->next_expiry().count()
              << " ns from epoch\n";
    std::cout << "  Next wheel expiry:   " << wheel_->next_expiry_ms()
              << " ms jiffy\n";

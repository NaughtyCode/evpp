...
public:
    ...
    virtual TimePoint read() const { return read_fn_ ? read_fn_() : clock_now_monotonic(); }

    // Clock source metadata
    const std::string& name()   const { return name_; }
    int                rating() const { return rating_; }
    uint32_t           freq_khz() const { return freq_khz_; }
    uint32_t           mult()   const { return mult_; }
    uint32_t           shift()  const { return shift_; }
    uint64_t           mask()   const { return mask_; }
    uint64_t           max_idle_ns() const { return max_idle_ns_; }
    unsigned long      flags()  const { return flags_; }

    // Flags
    bool is_continuous()   const { return (flags_ & kFlagContinuous) != 0; }
    bool is_valid_for_hres() const { return (flags_ & kFlagValidForHres) != 0; }
    bool is_unstable()     const { return (flags_ & kFlagUnstable) != 0; }
    bool suspend_nonstop()  const { return (flags_ & kFlagSuspendNonstop) != 0; }

    void set_rating(int r)       { rating_ = r; }
    void set_flags(unsigned long f) { flags_ = f; }
    void add_flag(unsigned long f)  { flags_ |= f; }
    void clear_flag(unsigned long f) { flags_ &= ~f; }
...
public:
    MonotonicClockSource()
        : ClockSource("monotonic", ClockSource::kRatingExcellent,
...
public:
    RealtimeClockSource()
        : ClockSource("realtime", ClockSource::kRatingGood,
...
public:
    ...
    TimePoint            next_event()    const { return next_event_; }
    const std::string&   name()          const { return name_; }
    int                  rating()        const { return rating_; }
    unsigned int         features()      const { return features_; }
    int64_t              min_delta_ns()  const { return min_delta_ns_; }
    int64_t              max_delta_ns()  const { return max_delta_ns_; }
    uint32_t             mult()          const { return mult_; }
    uint32_t             shift()         const { return shift_; }
    ...
    void set_rating(int r)                 { rating_ = r; }
    void set_features(unsigned int f)      { features_ = f; }
    void set_min_delta_ns(int64_t ns)       { min_delta_ns_ = ns; }
    void set_max_delta_ns(int64_t ns)       { max_delta_ns_ = ns; }

    bool has_feature(unsigned int f) const { return (features_ & f) != 0; }
...
public:
    ...
    TimePoint now_realtime() const  { return clock_now_realtime(); }
    TimePoint now_boottime() const  { return clock_now_boottime(); }
    TimePoint now_for(ClockId id) const {
        switch (id) {
            case ClockId::kMonotonic: return now();
            case ClockId::kRealtime:  return now_realtime();
            case ClockId::kBoottime:  return now_boottime();
    ...
    // Timekeeping state
    bool is_suspended() const { return suspended_.load(); }
    void set_suspended(bool s) { suspended_.store(s); }

    // Time offset tracking (for suspend/resume compensation)
    TimePoint suspend_time() const { return suspend_time_; }

...
inline const char* clock_id_name(ClockId id) {
    switch (id) {
        case ClockId::kMonotonic: return "monotonic";
        case ClockId::kRealtime:  return "realtime";
        case ClockId::kBoottime:  return "boottime";
        case ClockId::kTai:       return "tai";
        case ClockId::kRaw:       return "raw";
...
inline constexpr TimePoint ns_to_time(int64_t ns)   { return TimePoint(ns); }
inline constexpr TimePoint us_to_time(int64_t us)   { return TimePoint(us * kNsPerUs); }
inline constexpr TimePoint ms_to_time(int64_t ms)   { return TimePoint(ms * kNsPerMs); }
inline constexpr TimePoint sec_to_time(int64_t sec) { return TimePoint(sec * kNsPerSec); }

inline constexpr int64_t  time_to_ns(TimePoint t) { return t.count(); }
inline constexpr int64_t  time_to_us(TimePoint t) { return t.count() / kNsPerUs; }
inline constexpr int64_t  time_to_ms(TimePoint t) { return t.count() / kNsPerMs; }
inline constexpr int64_t  time_to_sec(TimePoint t) { return t.count() / kNsPerSec; }
...
inline constexpr bool time_before(TimePoint a, TimePoint b) { return a < b; }
inline constexpr bool time_after(TimePoint a, TimePoint b)  { return a > b; }
inline constexpr bool time_equal(TimePoint a, TimePoint b)  { return a == b; }

inline constexpr TimePoint time_min(TimePoint a, TimePoint b) { return a < b ? a : b; }
inline constexpr TimePoint time_max(TimePoint a, TimePoint b) { return a > b ? a : b; }
...
inline constexpr bool mode_is_absolute(TimerMode m)  { return (m & TimerMode::kRelative) == TimerMode::kAbsolute; }
inline constexpr bool mode_is_relative(TimerMode m)  { return (m & TimerMode::kRelative) == TimerMode::kRelative; }
inline constexpr bool mode_is_repeating(TimerMode m) { return (m & TimerMode::kRepeating) == TimerMode::kRepeating; }
inline constexpr bool mode_is_one_shot(TimerMode m)  { return (m & TimerMode::kRepeating) == TimerMode::kOneShot; }
inline constexpr bool mode_is_hard(TimerMode m)      { return (m & TimerMode::kHard) == TimerMode::kHard; }
inline constexpr bool mode_is_soft(TimerMode m)      { return (m & TimerMode::kSoft) == TimerMode::kSoft; }
inline constexpr bool mode_is_pinned(TimerMode m)    { return (m & TimerMode::kPinned) == TimerMode::kPinned; }
inline constexpr bool mode_is_deferrable(TimerMode m){ return (m & TimerMode::kDeferrable) == TimerMode::kDeferrable; }
...
inline const char* timer_state_name(TimerState s) {
    switch (s) {
        case TimerState::kInactive:  return "inactive";
        case TimerState::kArmed:     return "armed";
        case TimerState::kFiring:    return "firing";
        case TimerState::kCancelled: return "cancelled";
        default:                     return "unknown";
    }
}
...
struct TimerStats {
    ...
    void record_cancel()  { ++total_cancelled; }
    void record_restart() { ++total_restarted; }
    void record_overrun() { ++overrun_count; }
...
inline TimePoint clock_now_for(ClockId id) {
    switch (id) {
        case ClockId::kMonotonic: return clock_now_monotonic();
        case ClockId::kRealtime:  return clock_now_realtime();
        case ClockId::kBoottime:  return clock_now_boottime();
        case ClockId::kTai:
        case ClockId::kRaw:       return clock_now_raw();
        default:                  return clock_now_monotonic();
    }
}

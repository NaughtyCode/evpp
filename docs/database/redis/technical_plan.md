...
- Redis runtime 编译启用时，每个拥有 Lua VM 的线程都可以导出访问 `RedisClient` 相关的 API。
...
另外，现有 `MainThreadScriptVM` 对 Mongo/DB binding 的宏判断存在组合宏使用，Redis 模块应避免重复这种隐式判断。建议用非 cache 的 `ENGINE_REDIS_RUNTIME_ENABLED` 普通 CMake 变量控制 runtime 源文件列表，用 `ENGINE_REDIS_ENABLED` compile definition 表示当前 target 已编译 Redis runtime，二者都必须由 server/client target 在 include runtime 前显式设置。
...
`argv` 使用 `std::string` 保存参数，hiredis 调用时使用 `redisAsyncCommandArgv`，保证二进制安全。`redisAsyncCommandArgv` 会在调用栈内格式化命令缓冲区，所以 `argv` 的字符指针只需要在本次调用返回前有效；但 callback `privdata` 只是被 hiredis 保存为裸指针，pending token / request metadata 必须由 Redis 模块自行保证生命周期，直到 hiredis callback、超时、断线或 shutdown 去重流程确认不会再访问。
...
- 只有所有需要执行的握手步骤都成功后，`IsHealthy()` 才能返回 true，Redis 线程才可以 drain 普通 Redis 请求；未配置 password/username 且 `database = 0` 时，connect callback 成功后即可进入 healthy。
- AUTH 或 SELECT 失败应完成握手为 `kAuthError` 或 `kConnectionError`，设置 unhealthy，并按重连策略处理。
- AUTH/SELECT 握手命令由 `RedisClientThread` 内部直接发送，不构造成普通 `RedisRequest`，也不经过普通 `Command` 的 unsupported-command 校验；普通外部 API 和 Lua binding 仍必须拒绝 `AUTH` / `SELECT`。
...
7. worker healthy 且发送门槛满足时，从 `unsent_requests_` 取请求并调用 `redisAsyncCommandArgv`。
...
- Redis 线程在 drain 新请求、hiredis reply 释放 inflight 名额、发送失败释放名额、worker 重新 healthy、以及周期性 tick 时，都应尝试继续 flush 本 worker 的 `unsent_requests_`，直到 `unsent_requests_` 为空、worker 不 healthy、或达到 `max_inflight`。
...
- 普通 Redis 请求（外部请求和 Redis 私有 VM 发起的请求）只有在该 worker 已完成连接握手、AUTH/SELECT 成功且 `healthy = true` 后，才能发送给 hiredis；内部 AUTH/SELECT 握手命令不受该门槛影响。
- `queue_while_disconnected = true` 时，unhealthy worker 可以把已 accepted 但尚未发送的请求保留在 `request_queue_` 或 `unsent_requests_`；这些请求在排队期间仍按 accepted 时间计算 timeout，超时后完成为 `kTimeout`，不能等到重连后再发送。
...
2. RedisClient 拒绝新请求。
3. 唤醒所有 Redis 线程。
4. 每个 worker 对 `request_queue_` 和 `unsent_requests_` 中尚未发送的请求生成 shutdown 失败结果。
5. 显式 shutdown 时，每个 worker 对 pending requests 生成 `kShutdown` 失败结果；普通连接断开导致的 pending 失败按 13.2 生成 `kConnectionError`，不要在 shutdown 流程中混用。
...
导出接入点必须是面向 `ScriptVM&` 的通用 helper，例如 `script::ExportRedis(ScriptVM& vm, RedisLuaBindingContext context)`，不能只依赖 `MainThreadScriptVM::ExportRuntimeBindings()`。`RedisLuaBindingContext` 至少包含当前 VM 所属的 `AsyncResultDispatcher`、owner thread 标识，以及可选的内部 `preferred_worker_index`。`MainThreadScriptVM`、`DBScriptVM`、space VM、physics VM 和其他 owner thread 创建的 VM 都应在各自线程内初始化 dispatcher 后显式调用该 helper；`RedisClientScriptVM` 使用 Redis 私有 VM 专用 binding，同样需要自己的 dispatcher，但可以额外暴露 Redis 模块内部允许脚本访问的 public API 包装。
...
- `redis` 加载成功但连接失败：
  ...
  - `redis_required = false`：server 继续启动，并按重连策略后台重连。新请求按 5.1 的路由规则处理：无可用 worker 且 `queue_while_disconnected = false` 时同步拒绝为 `kDisconnected`，`queue_while_disconnected = true` 时可按上限进入 running worker 队列；已发送但未完成的 pending 请求按 13.2 完成为 `kConnectionError`。
...
`resources/config/server/server.json` 应显式保留默认空入口，避免依赖 `ServerConfig` 的 C++ 默认值导致运行时配置快照、Dump 输出和人工排查不一致：
...
1. `LoadServerFromFile` 先解析 `server.json`；`LoadServerFromString`、`Load`、`Reload` 和 `ValidateOnly` 也应复用同一套 Redis 入口校验和详细配置加载逻辑，不能让 dry-run 校验和真实加载走出不同结论。
...
`server_config_` 与 `redis_config_` 必须作为同一个配置快照更新：`LoadServerFromString`、`LoadServerFromFile`、`Reload` 在覆盖当前配置前要同步保存 `previous_redis_config_` 和 `previous_redis_loaded_`；`Rollback` 必须同时恢复 `server_config_`、`redis_config_`、`redis_loaded_` 和上一份 Redis 快照，避免回滚后 `server.redis` 指向旧路径但 `redis_config_` 仍是新内容。
...
- 当前 `ConfigManager::Diff` 只比较 runtime/server 字段；接入 Redis 时必须扩展签名以接收 old/new Redis 详细配置和 loaded flag，或增加独立 `DiffRedisConfig` helper 并在 `LoadServerFromString`、`Reload` 和 `Rollback` 构造同一个 `ConfigChangeSet` 时追加结果。
- Redis 详细配置 diff 只能在 Redis runtime 已编译且对应 Redis 配置已成功加载时比较；从 loaded 变为 unloaded 或从 unloaded 变为 loaded 时，也必须生成可让 `RedisClient` reload 逻辑识别的 change。
...
- `Dump()` / `DumpServer()` 只输出 `server.redis` 路径和 `redis_required`；如果需要导出 Redis 详细配置，应提供单独的 redacted dump helper，所有 password 字段必须输出 `<redacted>`。
...
- Redis 详细配置变化必须基于 `ConfigManager` 的 old/new Redis 缓存计算，不允许由 `RedisClient` 在 reload callback 中再次直接读取 `redis.json` 来推断变化。
...
- Redis runtime 已编译且 `server.redis` 从空变为非空：由 `ConfigManager` reload 流程加载并缓存 Redis config 后启动 `RedisClient`；若加载或启动失败，按新的 `redis_required` 规则处理 readiness 和错误日志。
...
基于当前 `Engine::Init` 现状，Redis 不应要求重排 logger、timer 或主 `EventLoop` 初始化；建议插入点：

1. 启动流程先完成 `ConfigManager::Load`，server config 和 Redis 详细配置缓存已按 10.4 的规则准备好。
2. `Engine::Init` 初始化 logger、profiler、timer 和主 `EventLoop`。
3. 主 `EventLoop` 可用后、Mongo / `DatabaseService` 初始化前，如果 Redis runtime 已编译且 `ConfigManager::IsRedisConfigLoaded()` 为 true，从 `ConfigManager` 读取缓存的 `RedisClientConfig` 并初始化 `RedisClient`。
4. 如果缓存的 Redis config 不可用，按 `server.redis`、Redis runtime 编译状态和 `redis_required` 保持 Redis 状态为 disabled/error，不创建 worker 集合。
5. 初始化 Mongo / `DatabaseService`。
6. 创建主 `MainThreadScriptVM`，后续初始化 physics / space 等可能拥有 Lua VM 的模块。
7. 导出 runtime bindings；Redis runtime 编译启用时，包含访问 `RedisClient` 的 Redis Lua binding。

Redis 详细配置由 `ConfigManager` 在 server config 加载阶段按 10.4 的规则解析和缓存，Engine 不应绕过 `ConfigManager` 再次直接读取 `redis.json`。Redis 启用时放在 Mongo / DatabaseService 之前初始化，是为了让后续 DB 线程、业务线程或脚本 VM 在创建时可以安全导出访问 `RedisClient` 的 Redis Lua binding；不要把 Redis 初始化移动到 `DatabaseService`、physics 或任一 VM 创建之后。Engine 读取 `server.redis_required` 后，转成 `RedisClientStartOptions::wait_for_initial_connect` 传给 `RedisClient::Initialize`：`redis_required = true` 时必须等待所有 `RedisClientThread` worker 的首次连接结果；`redis_required = false` 时只要求所有 Redis worker 基础设施启动成功。`server.redis` 为空且 `redis_required = false` 时不调用 `RedisClient::Initialize`；`server.redis` 为空但 `redis_required = true` 必须在配置校验阶段失败。Redis runtime 未编译且 `redis_required = false` 时也不调用 `RedisClient::Initialize`，即使 `server.redis` 保留了旧路径。
...
- 需要同步更新 `src/runtime/monitoring/admin_http.cc` 中的 `CleanupPhaseToString`，否则 admin cleanup/drain 接口无法正确输出 `RedisShutdown`。
...
- Redis shutdown 应先停止接收新请求，再让每个 `RedisClientThread` worker 把 `request_queue_`、`unsent_requests_` 和 `pending_requests_` 中的请求转成 `kShutdown`，最后唤醒目标 dispatcher；目标 dispatcher 是否执行 `kShutdown` 对应的 Lua/业务回调，由 owner thread 在本模块 shutdown 期间决定。
...
- 如果 `redis_required = false`，server 可以继续启动；在 `RedisClient` 已初始化且 worker 集合存在的前提下，新请求按 5.1 的路由和 `queue_while_disconnected` 规则处理。全部 worker 不可用且不允许断线排队时同步返回 `kDisconnected`，允许断线排队时可进入 running worker 队列；部分 worker 不可用时，未指定 `routing_key` 的请求继续路由到 healthy worker。Redis runtime 未编译、`server.redis` 为空或 Redis config 加载失败导致没有 worker 集合时，提交请求同步返回 `kNotRunning`。
...
每个 accepted request 记录 deadline；deadline 从 RedisClient accepted 时间开始计算，不从实际发送给 hiredis 的时间开始计算。

每个 Redis 线程通过自己的定时 event 检查本 worker 的超时：

- 尚未发送的超时请求从 `unsent_requests_` 删除；如果仍在 `request_queue_` 中，Redis 线程 drain 时必须先检查 deadline，过期则直接完成为 `kTimeout`，不得再发送。
- 已发送的超时请求从 pending map 删除。
...
- 超时完成必须同时覆盖 `request_queue_`、`unsent_requests_` 和 `pending_requests_` 三种状态；`request_queue_` 中无法随机删除的请求应在 drain 时按 request_id/deadline 识别并完成为 `kTimeout`，不得继续发送。
...
- `AUTH`、`SELECT`、Pub/Sub、blocking command、transaction command 等 unsupported command 返回 `kUnsupportedCommand`，不入队、不触发 completion；阻塞命令覆盖 `BLPOP`、`BRPOP`、`BLMOVE`、`BLMPOP`、`BZPOPMIN`、`BZPOPMAX`、`BZMPOP`、`WAIT`、`WAITAOF` 和带 `BLOCK` 选项的 `XREAD` / `XREADGROUP`。
- 配置驱动的 AUTH/SELECT 连接握手能正常执行，不被普通 unsupported-command helper 拦截；但外部 C++ API 和 Lua binding 直接提交 `AUTH` / `SELECT` 仍同步拒绝。
- 未配置 password/username 且 `database = 0` 时，connect callback 成功即可进入 healthy；配置 AUTH/SELECT 时，握手完成前 `IsHealthy()` 不能返回 true。
...
- Engine 初始化只在 `ConfigManager::IsRedisConfigLoaded()` 为 true 时消费缓存的 `RedisClientConfig`，不绕过 `ConfigManager` 二次读取 `redis.json`。
- Redis 初始化插入在当前 `Engine::Init` 主 `EventLoop` 创建之后、Mongo / `DatabaseService` 初始化之前；不要破坏 logger、timer 和主 `EventLoop` 的现有初始化顺序。
- Redis runtime 未编译、`redis_required = false` 且 `server.redis` 保留非空路径时，不加载 Redis 详细配置、不创建 worker，普通 VM 不导出 `redis` 表。
- Redis runtime 编译启用时，`MainThreadScriptVM`、`DBScriptVM`、space VM、physics VM 等普通 VM owner 都通过通用 `ScriptVM&` helper 导出访问 `RedisClient` 的 `redis` 表；禁用时这些 VM 都不导出。
...
- `ConfigManager::ValidateOnly`、`LoadServerFromFile`、`LoadServerFromString` 和 `Reload` 对 Redis 入口、`redis_required`、详细配置加载失败的结论一致。
- `ConfigManager::Dump` / Redis 详细配置 dump 不输出明文 password，reload diff 和日志也只输出 redacted 变化。
...
- 全部 worker unhealthy 且 `queue_while_disconnected = false` 时，新请求同步拒绝为 `kDisconnected`，不触发 completion。
...
- `max_inflight` 达到上限时请求停留在 `unsent_requests_`；reply、发送失败、重连 healthy 或 tick 释放/恢复发送条件后，worker 会继续 flush `unsent_requests_`。
...
- 已发送的 pending 请求在普通连接断开时完成为 `kConnectionError`；显式 `RedisClient::Shutdown()` 时完成为 `kShutdown`，两种路径不能混用。
- 普通连接断开且 `queue_while_disconnected = false` 时，`request_queue_` 和 `unsent_requests_` 中已 accepted 但尚未发送的请求完成为 `kConnectionError`，不会在重连后发送。
- hiredis 以 `reply == nullptr` 触发命令 callback，且 disconnect path 已先完成同一 request 时，late callback 不会二次完成或访问已释放对象。
- timed out 请求的 hiredis late callback 不触发 use-after-free。
- 新增 `RedisShutdown` 后，admin cleanup/drain 输出的 `cleanup_phase` 能正确显示 `RedisShutdown`，不落入 unknown/default 文本。
...
- `queue_while_disconnected = true` 且请求在 `unsent_requests_` 中超时后，worker 恢复 healthy 也不能再发送该请求。
...
6. 扩展 `ConfigManager` 的 Redis 缓存、diff、reload 和 rollback 逻辑。
7. 确认 client/mobile 不编译 Redis。
...
4. 实现 `bind/redis_bind.cc`，提供面向 `ScriptVM&` 的通用 `script::ExportRedis` helper。
5. Redis runtime 编译启用时，在 `MainThreadScriptVM`、`DBScriptVM`、space VM、physics VM 等 owner thread 的 VM 初始化流程中接入 `script::ExportRedis`，导出访问 `RedisClient` 的 `redis.command`、`redis.eval`、`redis.is_running`、`redis.is_healthy`。
6. Lua callback 结果通过各 owner thread 的 dispatcher 回到所属线程。
...
风险：shutdown、超时、连接断开时 accepted request 丢失，尤其是断线排队但尚未发送给 hiredis 的请求。

规避：所有 accepted request 必须处于 `request_queue_`、`unsent_requests_` 或 `pending_requests_` 三种状态之一；关闭、超时和 `queue_while_disconnected = false` 的连接断开都必须覆盖三种状态并统一生成失败结果。
...
### 17.5 hiredis 断线空 reply 二次完成

风险：hiredis 在断线、free async context 或清理 callback 队列时，可能以 `reply == nullptr` 调用命令 callback；如果 Redis 模块的 disconnect callback 已经把同一个 pending request 完成为 `kConnectionError`，空 reply callback 会形成第二条完成路径。

规避：所有 completion 都必须经过稳定 request token 的 `TryComplete` / `MarkCompleted` 入口。disconnect callback、timeout tick、shutdown、发送失败和 hiredis callback 共享同一套去重状态；空 reply callback 只在 token 尚未完成时生成 `kConnectionError`，否则仅记录 debug 日志并释放 callback 侧资源。

### 17.6 client 误编译 Redis

风险：CMake cache 导致 client 继承 server 的 `ENGINE_REDIS_ENABLED`。

规避：使用 include runtime 前设置的非 cache `ENGINE_REDIS_RUNTIME_ENABLED` 控制源列表；client/mobile 明确设为 OFF，且不添加 `ENGINE_REDIS_ENABLED` compile definition。

### 17.7 多 worker 路由和顺序风险
...
- RedisClientScriptVM。
- Redis runtime 编译启用时，各普通 VM owner 通过通用 helper 导出的 Lua `redis.command`、`redis.eval`、`redis.is_running`、`redis.is_healthy`。
- 跨线程 dispatcher。

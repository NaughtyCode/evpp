...
**客户端同样需要身份标识，但语义不同:** 客户端需要的是设备 ID、平台信息（OS 版本/GPU 型号/驱动版本）、用户 ID、会话 ID——这些用于崩溃报告分组、遥测关联、多设备同步。当前同样没有任何配置支持。

---

## 12. 开发期与发布期配置差异

作为一个双用途（客户端+服务器）游戏引擎，配置系统必须支撑两个截然不同的生命周期阶段——以及两个截然不同的运行时角色。当前实现将 dev/prod 差异分散在编译期宏、默认值、CLI 参数和手写 JSON 中，没有统一的"配置 profile"概念；客户端特有的 dev/prod 差异（如调试渲染、离线模式）则完全不在配置系统覆盖范围内。
...
以下是游戏引擎在开发阶段的关键配置需求（涵盖服务器端和客户端），以及当前实现的支持程度：
...
当前引擎定位为双用途引擎，但配置系统在客户端侧仍然是占位符（见 §1.5），也没有预留"导出客户端可读格式"的能力。当客户端团队需要一份与服务器同步的配置时（如物品 ID 枚举、技能参数），必须手工维护两份配置——极易产生双端不一致的 Bug。
...
| 特性 | 服务器 | 客户端 |
|------|--------|--------|
| **热更触发方式** | 手动（CLI 调用 / 代码触发，SIGHUP 和 HTTP endpoint 均缺失，见 §8.2） | 自动（用户在设置菜单中拖动滑块） |
...
客户端运行在高度多样化的硬件上（集成显卡到 RTX 5090，4GB RAM 到 64GB RAM，不同厂商的 GPU 驱动），而服务器通常运行在预先规划的实例规格上（云实例、裸金属或虚拟机，硬件配置在部署前已知）。这一根本差异产生了两个服务器端不存在的配置需求：
...
| # | 问题 | 位置 | 影响 |
|---|------|------|------|
| P3-1 | `callbacks_mutex_` 的 `shared_mutex` 收益有限，可简化为 `std::mutex` | `config.h:323` | 回调注册是低频操作，shared_mutex 相对 mutex 的额外开销无实际收益 |
...
> **注意:** 此附录仅覆盖服务器端数据流。客户端数据流尚不存在（`ClientConfig` 仅 1 个字段，见 §1.5），客户端配置加载路径待定义。
